...
//   Tutorial application -- step15dv.cpp
...
GetPenColor(TWindow* parent, TLine& line)
{
  ...
  if (TChooseColorDialog(parent, colors).Execute() != IDOK)
    return false;
  line.SetPen(colors.Color, line.QueryPenSize());
  return true;
}
...
TDrawView::EvLButtonDown(uint modKeys, TPoint& point)
{
  ...
  if (DragDC) {
    SetCapture();
    Pen = new TPen(Line->QueryColor(), Line->QueryPenSize(), PS_INSIDEFRAME);
...
//
void
TDrawView::Paint(TDC& dc, bool /*erase*/, TRect& /*clip*/)
{
