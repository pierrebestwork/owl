...
public:
    ...
    bool Add(const T& t) {
        if ( d_delta > 0 && size() >= capacity()) {
            reserve(size() + d_delta);
        }
        push_back(t);
        return true;
    }
    ...
    {
        PRECONDITION( loc >= d_lower && ZeroBase(loc) < size() );
        return at(ZeroBase(loc));
    }
    ...
    void Grow( int loc ) //increases the sizes, but only if needed, to ensure that loc is a valid position into the array
    {
      ...
      if ( new_size >= (int)capacity() ) { //if new_size>max number of items, then resize
        ...
        reserve(new_size); //hgx 2010-06-11
      }
      else if (new_size < 0) {
        PRECONDITION(d_delta>0); //will help while debugging
    ...
    int IsFull() const
    {
        return d_delta == 0 && size() >= capacity();
    }    
