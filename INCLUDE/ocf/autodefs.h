...
_OCFFUNC(const void *) DynamicCast(const void * obj, const __OWL_TYPEINFO& src,
	                                                const __OWL_TYPEINFO& dst);

_OCFFUNC(const void *) MostDerived(const void * obj, const __OWL_TYPEINFO& src);
//
// Global function to invalidate external references, class model-independent
//
_OCFFUNC(void) SendObituary(const void * obj, const __OWL_TYPEINFO& typeInfo);
...
class _AUTOCLASS TAutoDetach { // default base for detach notification objects
  protected:
    void Notify(int offset, const __OWL_TYPEINFO& typeInfo);  // sends obituary
...
typedef TAutoIterator* (*TAutoIteratorBuild)(ObjectPtr obj,
                                             ...
                                             IUnknown* owner, __OWL TLangId lang);
...
//
class _OCFCLASS TXAuto : public __OWL TXBase {
  public:
    enum TError {
      xNoError,                 //  0
...
class _OCFCLASS TAutoClass : public TAutoType {
  public:
    TAutoClass(TAutoSymbol* table, TAutoSymbol* classSymbol,
               const __OWL_TYPEINFO& typeInfo, TAggregator aggregator=0);
   ~TAutoClass();

    short         CountCommands();    // forces counts to be set on first call
    TAutoSymbol*  Lookup(LPTSTR name, __OWL TLangId lang, short symFlags, long & id);
    TAutoSymbol*  LookupArg(LPTSTR name, __OWL TLangId lang, TAutoSymbol* cmd,
                            long & retid);
    ...
    const __OWL_TYPEINFO&      GetTypeInfo() const;
    LPCTSTR								GetName(__OWL TLangId lang = __OWL TLocaleString::UserDefaultLangId) const;
    LPCTSTR								GetDoc (__OWL TLangId lang = __OWL TLocaleString::UserDefaultLangId) const;
  ...
  protected:
    ...
    const __OWL_TYPEINFO& TypeInfo;// for validating type of data pointers
...
struct _OCFCLASS TAutoCurrency {
  ...
  //
  struct _OCFCLASS TAutoBool {
    TAutoBool() : B(0) {}
    TAutoBool(bool b) : B(b) {}
    TAutoBool(__OWL TBool b) : B(static_cast<bool>(b)) {}
    operator bool() {return B;}
    operator __OWL TBool() {return static_cast<__OWL TBool>(B);}
    void operator =(bool b) {B = b;}
    void operator =(__OWL TBool b) {B = static_cast<bool>(b);}
    bool B;
    static TAutoType ClassInfo;
  };
  ...
  struct TAutoBoolRef {  //
    ...
    TAutoBoolRef(__OWL TBool* p) : P((bool*)p) {}
    operator bool*() {return P;}
    void operator =(bool* p) {P = p;}
    void operator =(__OWL TBool* p){P = (bool*)p;}
    bool* P;
    static TAutoType ClassInfo;
  };
#else
...
//----------------------------------------------------------------------------
// Automated class __OWL_TYPEINFO flags - corresponding to OLE 2.02 definitions
//    flags set in END_AUTOCLASS, END_AUTOAGGREGATE, END_AUTOEVENTCLASS macros
//

//
// Flags set on any autoclass, but apply only to enclosing coclass __OWL_TYPEINFO.
...
struct _OCFCLASS TAutoSymbol {
  __OWL TLocaleString Name;         // name of symbol, initially untranslated
  __OWL TLocaleString Doc;          // documentation string, initially untranslated
...
struct _OCFCLASS TObjectDescBase {   // ONLY for use when storing in TAutoVal union
  ...
  TAutoClass* Class;       // class descriptor, contains __OWL_TYPEINFO
...
//
class _OCFCLASS TAutoString : public __OWL TString {
  public:
    TAutoString(const char* s = 0) : __OWL TString(s) {}
    ...
    TAutoString(const TAutoString& src) : __OWL TString(src.S) {++*S;}
...
class _OCFCLASS TAutoVal {
  public:
    ...
    TAutoVal(__OWL TBool              i){ *this = i; };
    TAutoVal(__OWL TBool*             p){ *this = p; };
#if !defined(BI_UNIQUE_BOOL)
    TAutoVal(TAutoBool          i){ *this = i; };
    TAutoVal(TAutoBoolRef       p){ *this = p; };
#endif
    TAutoVal(const owl_string&	s){ *this = s; };
#pragma warn -inl
    TAutoVal(TAutoString        s){ *this = s; };
#pragma warn .inl

#if defined(BI_COMP_MSC)
    TAutoVal(__OWL TString						s){ *this = s; };
#endif
...
//    TAutoVal(LARG_INTEGER       v){ *this = v; };
    ...
    operator TAutoDate*();
    operator owl_string();
    operator __OWL TUString*();
    operator __OWL TString();
    ...
    void operator =(__OWL TBool              i);
    void operator =(__OWL TBool*             p);
#if !defined(BI_UNIQUE_BOOL)
    void operator =(TAutoBool          i);
    void operator =(TAutoBoolRef       p);
#endif
    void operator =(const owl_string& s);
    void operator =(TAutoString        s);
#if defined(BI_COMP_MSC)
    void operator =(__OWL TString 		 s);
#endif
    ...
    __OWL TLangId			GetLanguage() const;      // Retrieve language from pointer types
    __OWL TString			StrVal();                 // Returns internal string pointer if atString
    ...
    unsigned short Reserved1;
		...
		union {
			...
			struct {                      // Added info when loaned BSTR ownership
				BSTR			Val;
				__OWL TUString* Holder;     // String holder sharing current BSTR
			} s;
...
template <class T> struct TAutoEnumVal {
  __OWL TLocaleString Name;         // Name of symbol, enumeration text
...
class TAutoEnum : public TAutoType {
  public:
    ...
    virtual bool Convert(TAutoVal& numVal, __OWL TLangId langId) = 0;
...
template <class T> class TAutoEnumT : public TAutoEnum {
  public:
    ...
    bool Convert(TAutoVal& numVal, __OWL TLangId langId);
...
class _ICLASS TServedObject : public TServedCOM {
  public:
   ...
   ~TServedObject();
    ...
    __OWL TLangId 							ReqLang;			// language requested by caller
  ...
  private:
    ...
    TServedObject*        Next;
    TServedObject**				Prev;
    // need to access local LANGID for __OWL_TYPEINFO, either as member or as global
...
class _OCFCLASS TAutoIterator : public IEnumVARIANT {
  ...
  protected:
    ...
    unsigned			RefCnt;
    __OWL TLangId				Lang;
    TAutoIterator(TAutoCreator& creator, IUnknown* owner, __OWL TLangId lang); // called from derived
...
class _OCFCLASS TAutoProxy {
  public:
   ~TAutoProxy();
    ...
    //
    void        MustBeBound();
    bool        IsBound()  {return __OWL ToBool(That != 0);}
    ...
    // Locale support
    //
    void        SetLang(__OWL TLangId lang) {Lang = lang;}
  ...
  protected:
    TAutoProxy(__OWL TLangId lang) : That(0), Lang(lang) {}
...
inline const __OWL_TYPEINFO&  TAutoClass::GetTypeInfo() const {
  return TypeInfo;
}
inline LPCTSTR TAutoClass::GetName(__OWL TLangId id) const {
  return ClassSymbol->Name.Translate(id);
}
inline LPCTSTR TAutoClass::GetDoc (__OWL TLangId id) const {
...
:
  __OWL TString(val.operator __OWL TString())
//#if defined(BI_COMP_MSC)
...
#if !defined(BI_UNIQUE_BOOL)
inline TAutoVal::operator TAutoBool()    {return TAutoBool(operator __OWL TBool());}
inline TAutoVal::operator TAutoBoolRef() {return TAutoBoolRef(operator __OWL TBool*());}
#endif
...
inline void TAutoVal::operator =(__OWL TBool           v){vt=atBool;   boolVal = short(v ? -1 : 0);}
#if !defined(BI_UNIQUE_BOOL)
...
inline void TAutoVal::operator =(__OWL TBool*         p){vt=atBool+atByRef;  pbool=(VARIANT_BOOL*)p;}
#if !defined(BI_UNIQUE_BOOL)
...
//
//
inline __OWL TString TAutoVal::StrVal() {
...
inline bool TAutoCommand::TestFlag(int mask) {return __OWL ToBool((Attr&mask)==mask);}
...
//
template <class T> bool
TAutoEnumT<T>::Convert(TAutoVal& numVal, __OWL TLangId langId)
{
  T val;
...
//!BGM  with some templatized functions.  Deferred for Ebony.
#pragma warn -inl
#if defined(BI_STD_SPECIALIZATION)
template<>
#endif
inline bool
TAutoEnumT<LPCTSTR>::Convert(TAutoVal& numVal, __OWL TLangId langId)
{
...
{
	__OCF_GLOBAL::SendObituary(this, typeid(TAutoBase));
}

inline void TAutoDetach::Notify(int offset, const __OWL_TYPEINFO& typeInfo)
{
  __OCF_GLOBAL::SendObituary((char*)this-offset, typeInfo);
}
//____________________________________________________________________________
