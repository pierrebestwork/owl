...
class _OCFCLASS TOcFormatName {
  public:
    ...
    bool operator ==(const TOcFormatName& other) const {return __OWL ToBool(&other == this);}
  ...
  private:
    owl_string Id;          // internal clipboard format name
    owl_string Name;        // external name of clipboard format (used in list box)
    owl_string ResultName;  // string to be used in the dialog help text
};
...
//
class _OCFCLASS TOcNameList : protected __OWL TIPtrArray<TOcFormatName*>
{
  public:
    TOcNameList();
   ~TOcNameList();
  ...
  private:
    typedef __OWL TIPtrArray<TOcFormatName*> Base;
};

#if defined(BI_IMPEXP_TEMPLATES)
# if defined(_OCFDLL) || defined(BI_APP_DLL)
  //
  // Export template of __OWL TIPtrArray<TOcFormatName*> when building ObjectWindows
  // DLL and provide import declaration of DLL instance for users of the class.
  //
  template class __OWL TIPtrArray<TOcFormatName*>;
# endif
#endif
...
class _OCFCLASS TOcRegistrar : public TRegistrar {
  public:
    TOcRegistrar(__OWL TRegList& regInfo, TComponentFactory callback,
                                owl_string& cmdLine, __OWL TRegLink* linkHead = 0,
...
class _ICLASS TOcApp : public TUnknown,
  ...
  public:
    ...
    owl_string  GetName() const {return (LPCTSTR)Name;}
    ...
    //
    bool     UnregisterClass(const owl_string& progid);
    void     EvResize();
    void     EvActivate(bool);
    bool     EvSetFocus(bool set) {return HRSucceeded(BServiceI->OnSetFocus(set));}
    bool     RegisterClass(const owl_string& progid, BCID classId, bool multiUse);
...
//!CQ    uint32 ForwardEvent(int eventId, const void * param);
//!CQ    uint32 ForwardEvent(int eventId, uint32 param = 0);
...
class _OCMCLASS TOcAppFrameHost {
  public:
    ...
    virtual bool  EvOcAppFrameRect(__OWL TRect * rect) = 0;
    virtual bool  EvOcAppBorderSpaceReq(__OWL TRect * rect) = 0;
    virtual bool  EvOcAppBorderSpaceSet(__OWL TRect * rect) = 0;
