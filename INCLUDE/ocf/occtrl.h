...
// change from protected IBControlSite to public IBControlSite -> c++ sintax
...
class _ICLASS TOcControl : public TOcPart, protected IBControlSite {
  public:
    ...
    //
    bool        Init(TOcInitInfo * initInfo, __OWL TRect pos);
    ...
    void        SetUserName(__OWL TString& name);
  ...
  protected:
   ~TOcControl();
    ...
    HRESULT _IFUNC TransformCoords(__OWL TPointL * lpptlHimetric,
                            __OWL TPointF * lpptfContainer, DWORD flags);
  ...
  DECLARE_AUTOCLASS(TOcControl)
    ...
    //AUTOPROP(Name,        GetUserName,  SetUserName,    __OWL TString, public:)
#pragma warn -inl
        AUTOPROPBUG(Name,        GetUserName,  SetUserName, __OWL TString, public:) // yura 05/26/98
#pragma warn .inl
...
class _ICLASS TOcxView : public TOcView {
  public:
    TOcxView(TOcDocument& doc, __OWL TRegList* regList=0, IUnknown* outer=0);
   ~TOcxView();
    ...
    void        SetDisplayName(__OWL TString& Name);
    void        SetScaleUnits(__OWL TString& ScaleUnits);
  ...
  protected:
    ...
    __OWL TString&    GetDisplayName();
    __OWL TString&    GetScaleUnits();
    ...
    void        SetAmbDisplayName(__OWL TString& Name);
    void        SetAmbScaleUnits(__OWL TString& ScaleUnits);
  ...
  DECLARE_AUTOCLASS(TOcxView)
    ...
    //AUTOPROP(DisplayName,      GetDisplayName,    SetAmbDisplayName,    __OWL TString, public:)
#pragma warn -inl
    AUTOPROPBUG(DisplayName,   GetDisplayName,    SetAmbDisplayName, __OWL TString, public:) // yura 05/26/98
#pragma warn .inl
    //AUTOPROP(ScaleUnits,       GetScaleUnits,     SetAmbScaleUnits,     __OWL TString, public:)
#pragma warn -inl
    AUTOPROPBUG(ScaleUnits,    GetScaleUnits,     SetAmbScaleUnits,  __OWL TString, public:) // yura 05/26/98
#pragma warn .inl
...
struct TCtrlTransformCoords : public TCtrlEvent {
  ...
  TCtrlTransformCoords(TOcControl* ctl, __OWL TPointL* him, __OWL TPointF* pcont,
                       DWORD flgs) :
                       TCtrlEvent(ctl, 0/*IID for this event??*/),
                       lpptHimetric(him), lpptContainer(pcont), flags(flgs)
                       {}

  __OWL TPointL *lpptHimetric;
  __OWL TPointF *lpptContainer;
  DWORD     flags;
};
