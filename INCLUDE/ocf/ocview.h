...
//
class _OCFCLASS TOcFormatList : public __OWL TIPtrArray<TOcFormat*> {
  public:
    TOcFormatList();
   ~TOcFormatList();
    ...
    uint	Count() const												{	return Base::Size();}
  ...
  private:
    typedef __OWL TIPtrArray<TOcFormat*> Base;
};
...
class _ICLASS TOcView : public TUnknown,
  ...
  public:
    ...
    TOcView(TOcDocument& doc, __OWL TRegList* regList=0, IUnknown* outer=0);
    ...
    // Clipboard support
    //
    bool         RegisterClipFormats(__OWL TRegList& regList);
    bool         BrowseClipboard(TOcInitInfo& initInfo);
    bool         PasteNative(TOcInitInfo& init, __OWL TPoint * where = 0);
    ...
    // View related
    //
    __OWL TPoint       GetOrigin() const {return Origin;}
    __OWL TRect        GetWindowRect() const;
  ...
  protected:
    ...
    HRESULT  _IFUNC GetFormat(uint, TOcFormatInfo *);

    // IBWindow implementation
    //
    HWND     _IFUNC GetWindow();
    HRESULT  _IFUNC GetWindowRect(__OWL TRect * r);
    LPCOLESTR _IFUNC GetWindowTitle();
    void     _IFUNC AppendWindowTitle(LPCOLESTR title);
    HRESULT  _IFUNC SetStatusText(LPCOLESTR text);
    HRESULT  _IFUNC RequestBorderSpace(const __OWL TRect *);
    HRESULT  _IFUNC SetBorderSpace(const __OWL TRect *);
    ...
    //
    HRESULT  _IFUNC Drop(TOcInitInfo *, __OWL TPoint *, const __OWL TRect *);
    void     _IFUNC DragFeedback(__OWL TPoint *, const __OWL TRect *, TOcMouseAction, uint, HRESULT& hr);
    HRESULT  _IFUNC Scroll(TOcScrollDir scrollDir);
    HRESULT  _IFUNC GetScrollRect(__OWL TRect *);

    // IBContainer implementation
    //
    HRESULT  _IFUNC FindDropDest(__OWL TPoint *, IBDropDest * *);
    HRESULT  _IFUNC AllowInPlace();
    HRESULT  _IFUNC BringToFront();

  protected:
    //!CQ hacked in to support occtrl for now
    uint32 ForwardEvent(int eventId, const void * param);
...
//!CQ ripped out    HWND            Win;          // Actual app container window
    ...
    __OWL TString						OrgTitle;     // Original window title string
    int								LinkFormat;   // adjustment for format count
    __OWL TRegList*					RegList;
...
struct TOcViewPaint {
  ...
  __OWL TString*  Moniker;         // moniker if any
...
class TOcPartSize {
  public:
    ...
    TOcPartSize(__OWL TRect rect, bool selection = false, __OWL TString* moniker = 0, void* userData = 0)
    :
      PartRect(rect),
      Selection(selection),
      Moniker(moniker),
    ...
    TOcPartSize(bool selection, __OWL TString* moniker = 0, void* userData = 0)
    :
      PartRect(0, 0, 0, 0),
      Selection(selection),
      Moniker(moniker),
  ...
  public:
    __OWL TRect			PartRect;     // rect enclosing the whole/part of the embedded object
    bool			Selection;    // whether we want rect for the whole or part(selection)
    __OWL TString*	Moniker;      // we want the selection rect for moniker
    void*			UserData;     // User data
};
...
class TOcItemName {
  ...
  public:
    __OWL TString Name;        // Item moniker
...
class TOcFormatData {
  public:
    TOcFormatData(TOcFormat& format, void* userData = 0, HANDLE handle = 0,
                  bool paste = false, __OWL TPoint * where = 0)
    :
  ...
  public:
    ...
    __OWL TPoint *	Where;   // Where to drop this data
};
...
class _OCFCLASS TOcScaleFactor {
  public:
    TOcScaleFactor();
    TOcScaleFactor(const __OWL TRect& siteRect, const __OWL TSize& partSize);
  ...
  public:
    __OWL TSize SiteSize;
    __OWL TSize	PartSize;
};

_OCFCFUNC(__OWL ipstream&) operator >>(__OWL ipstream& is, TOcScaleFactor& sf);
_OCFCFUNC(__OWL opstream&) operator <<(__OWL opstream& os, const TOcScaleFactor& sf);
...
class _OCFCLASS TOcSiteRect {
  public:
    TOcSiteRect();
    TOcSiteRect(TOcPart *part, const __OWL TRect& rect);
    __OWL TRect			Rect;
    TOcPart*	Part;
};
...
class _OCMCLASS TOcContainerHost {
  public:
    ...
    virtual bool		EvOcViewBorderSpaceReq(const __OWL TRect * rect) = 0;
    virtual bool		EvOcViewBorderSpaceSet(const __OWL TRect * rect) = 0;
    ...
    // From site (TOcPart)
    //
//    virtual bool   EvOcViewGetSiteRect(__OWL TRect * rect) = 0;
//    virtual bool   EvOcViewSetSiteRect(__OWL TRect * rect) = 0;
...
class _OCMCLASS TOcCtrlContainerHost {
  public:
    ...
    virtual bool   EvOcAmbientGetDisplayName(__OWL TString** name) = 0;
    virtual bool   EvOcAmbientGetScaleUnits(__OWL TString** units) = 0;
    ...
    virtual bool   EvOcAmbientSetDisplayName(__OWL TString* name) = 0;
    virtual bool   EvOcAmbientSetScaleUnits(__OWL TString* units) = 0;
...
class _OCMCLASS TOcContainerHostMsg : public TOcContainerHost {
  public:
    ...
    virtual bool		EvOcViewBorderSpaceReq(const __OWL TRect * rect);
    virtual bool		EvOcViewBorderSpaceSet(const __OWL TRect * rect);
    ...
    // From site (TOcPart)
    //
//    virtual bool   EvOcViewGetSiteRect(__OWL TRect * rect);
//    virtual bool   EvOcViewSetSiteRect(__OWL TRect * rect);
...
{
  return __OWL ToBool(Options & option);
}
