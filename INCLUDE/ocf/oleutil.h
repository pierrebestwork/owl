...
class TBaseClassId : public TClassId {
  public:
    TBaseClassId(__OWL TRegList& regInfo, int maxAlloc = 256);
...
#define IUNKNOWN_FORWARD(cls, outer) \
  ...
  ulong _IFUNC cls::AddRef()  { return outer->AddRef(); } \
  ulong _IFUNC cls::Release() { return outer->Release(); }
