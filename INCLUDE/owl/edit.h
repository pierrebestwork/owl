...
class _OWLCLASS TEdit : public TStatic {
  public:
   ...
   ~TEdit();
    ...
    virtual int Search(uint startPos, LPCTSTR text,
...
//    HLOCAL  GetHandle() const;          //Who wrote this ????????????
...
//#endif
  ...
  //DECLARE_STREAMABLE(_OWLCLASS, NS_OWL::TEdit, 1);
  DECLARE_STREAMABLE_OWL(NS_OWL::TEdit, 1);
};
// define streameable inlines (VC)
DECLARE_STREAMABLE_INLINES( NS_OWL::TEdit );
...
inline int TEdit::ScrollText(int how){
  uint32 ret = SendMessage(EM_SCROLL, (WPARAM)(INT)how, 0);
