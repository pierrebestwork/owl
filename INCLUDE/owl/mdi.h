...
class _OWLCLASS TMDIClient : public virtual TWindow {
  ...
  //DECLARE_STREAMABLE(_OWLCLASS, NS_OWL::TMDIClient, 1);
  DECLARE_STREAMABLE_OWL(NS_OWL::TMDIClient, 1);
  
  friend class TMDIFrame;  // To allow access to private ctor
};
// define streameable inlines (VC)
DECLARE_STREAMABLE_INLINES( NS_OWL::TMDIClient );
...
class _OWLCLASS TMDIFrame : virtual public TFrameWindow {
  ...
  DECLARE_STREAMABLE_OWL(NS_OWL::TMDIFrame, 1);
};
// define streameable inlines (VC)
DECLARE_STREAMABLE_INLINES( NS_OWL::TMDIFrame );
