...
class _OWLCLASS TPrinterAbortDlg : public TDialog {
  public:
    TPrinterAbortDlg(TWindow* parent, TResId resId, LPCTSTR title,
                     LPCTSTR device, LPCTSTR port,
                     HDC prnDC = HDC(-1));
...
class _OWLCLASS TPrintout : public TStreamableBase {
  ...
  protected_data:
    _TCHAR *     Title;         // The title of this printout.
  ...
  //DECLARE_STREAMABLE(_OWLCLASS, NS_OWL::TPrintout, 1);
  DECLARE_STREAMABLE_OWL(NS_OWL::TPrintout, 1);
};
// define streameable inlines (VC)
DECLARE_STREAMABLE_INLINES( NS_OWL::TPrintout );
...
class _OWLCLASS TPrinter: public TStreamableBase {
  ...
  protected:
    ...
    virtual void     SetPrinter(LPCTSTR driver, LPCTSTR device, LPCTSTR output);
  ...
  //DECLARE_STREAMABLE(_OWLCLASS, NS_OWL::TPrinter, 2);
  DECLARE_STREAMABLE_OWL(NS_OWL::TPrinter, 2);
};
// define streameable inlines (VC)
DECLARE_STREAMABLE_INLINES( NS_OWL::TPrinter );
...
//
inline LPCTSTR TPrintout::GetTitle() const {
  return Title;
}
