...
#if defined(BI_PLAT_MSW) && defined(BI_NEED_TCHAR)
#  if defined(BI_COMP_BORLANDC)
inline int _OWLENTRY _tcscmp(const _TCHAR* s1, const _TCHAR* s2)
                   {return lstrcmp(s1, s2);}
inline int _OWLENTRY _tcsicmp(const _TCHAR* s1, const _TCHAR* s2)
...
#if defined(BI_DBCS_SUPPORT) && defined(BI_PLAT_MSW) && !defined(_UNICODE)
  inline UINT CharSize(const _TCHAR* s) {return ::AnsiNext(s) - s;}
#else
  inline UINT CharSize(const _TCHAR*) {return sizeof(_TCHAR);}
#endif
...
inline LPWSTR OwlA2WHelper(LPWSTR lpw, LPCSTR lpa, int nChars, uint acp)
{
...
inline LPSTR OwlW2AHelper(LPSTR lpa, LPCWSTR lpw, int nChars, uint acp)
{
  // verify that no illegal character present
...
#   if defined(BI_COMP_BORLANDC) && (__BORLANDC__ < 0x530)
      _OWLFUNC(wchar_t*) _tcscpy(wchar_t* dst, const wchar_t* src);
      _OWLFUNC(size_t)   _tcslen(const wchar_t* s);
#   endif
#  endif
