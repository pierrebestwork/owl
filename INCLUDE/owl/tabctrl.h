...
class _OWLCLASS TTabControl : public TControl {
  ...
  private:
    ...
    void        PaintTab(TDC& dc, const TRect& tbRect, LPCTSTR txt,
                         bool clip = false);
...
//!BB  void        SetTabRectsStacked(int firstTab);
...
//!BB  DECLARE_STREAMABLE(_OWLCLASS, NS_OWL::TTabControl, 1);
};
...
{
  return Insert(TTabItem(tabText), GetCount());
}
