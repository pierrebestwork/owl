...
class _OWLCLASS TWindow : virtual public TEventHandler,
  ...
  public:
    ...
    uint              SetTimer(UINT_PTR timerId, uint timeout, TIMERPROC proc = 0);
    ...
    // Misc
    //
    bool              WinHelp(LPCTSTR helpFile,
                              uint    command,
                              uint32  data);
    void              AssignContextMenu(TPopupMenu* menu);
    TPopupMenu*       GetContextMenu() const;
    int               MessageBox(LPCTSTR text,
                                 LPCTSTR caption = 0,
                                 uint    type     = MB_OK);
    int               MessageBox(uint    resId, 
                                 LPCTSTR caption = 0,
                                 uint    type = MB_OK,
                                         ...);
  ...
  public:
    ...
    typedef __OWL_GLOBAL::TXWindow TXWindow;        // Exceptions moved to global scope
#endif
...
//  DECLARE_STREAMABLE(_OWLCLASS, NS_OWL::TWindow, 3);
  DECLARE_STREAMABLE_OWL(NS_OWL::TWindow, 3);
};  // class TWindow

// define streameable inlines (VC)
DECLARE_STREAMABLE_INLINES( NS_OWL::TWindow );
...
#if defined(OWL2_COMPAT)
    static owl_string Msg(TWindow* win, uint resourceid);
#endif
...
inline TWindow* GetObjectPtr(HWND hWnd) {
  return NS_OWL::GetWindowPtr(hWnd);
}
#endif

#if defined(__TRACE) || defined(__WARN)
_OWLCFUNC(NS_OWL_STREAMS::ostream&) operator <<(NS_OWL_STREAMS::ostream& os, const TWindow& w);
#endif
...
// Wrapper for Windows API.
//
inline bool TWindow::PostMessage(uint msg, TParam1 p1, TParam2 p2){
  PRECONDITION(GetHandle());
...
//
inline int
TWindow::GetDlgItemText(int childId, LPTSTR text, int max) const
{
  PRECONDITION(GetHandle());
...
//
inline int
TWindow::MessageBox(LPCTSTR text,
                    LPCTSTR caption,
                    uint    type)
{
...
{
  return NS_OWL::GetWindowPtr(hWnd, GetApplication());
}
...
//
inline void TWindow::EvDevModeChange(LPTSTR /*device*/)
{
  DefaultProcessing();
}
...
//
inline uint TWindow::EvGetDlgCode(MSG *)
{
...
//
inline void TWindow::EvWinIniChange(LPTSTR /*section*/)
{
  DefaultProcessing();
}
...
//
inline void TWindow::EvWindowPosChanged(WINDOWPOS & /*windowPos*/)
{
  DefaultProcessing();
}
