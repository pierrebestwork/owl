...
//
// Many thanks to Riho Ellermaa, Michael Mogensen, Jürgen Welzenbach,
...
class OWLEXTCLASS TBmpMenu {
	public:
  	TBmpMenu(__OWL TModule* module = 0);
  	~TBmpMenu();
	...
	public:
  	...
  	bool FindKeyboardShortcut(uint nChar, uint nFlags, HMENU hMenu, LRESULT& lRes);
	  // To be called from the TWindow EvMenuChar(..)
  	// Call the base class if it return false

	  int PreGetHintText(HMENU hmenu, uint id, LPTSTR buf, int size, __OWL THintText hintType);
	...
	protected:
	  ...
	  void DrawCheckmark(__OWL TDC& hdc, int x, int y, bool enable, bool select);
		void DrawRadioDot(__OWL TDC& dc, int x, int y, bool enable, bool select);
	...
	protected:
  	...
  	bool        bIsPopup;         // true if the remap menu is a popup menu
		__OWL TSize       ImageSize;        // the size that is used for all bitmaps
		__OWL TImageList* ImageList;        // contains all used bitmaps (each bmp one time)
		__OWL TBitmap*    BackBitmap;       // background bitmap
	  uint        RadioBitmapId;    // resource id of radio dot bitmap
		uint        CheckBitmapId;    // resource id of checkbox bitmap
		__OWL TFont*      MenuFont;         // MenuFont to display text in all menus
	  __OWL TColor      MaskColor;        // Color of pixels that will be mask to the menu
	  __OWL TColor      Old3dFaceColor;
  	__OWL TModule*    Module;

	private:
  	__OWL TIPtrArray<TBmpMapEntry*>*			MapEntries;
	  __OWL TIntArray*                      ExcludedBitmaps;
  	__OWL TIPtrArray<TBmpMenuItem*>*      MenuItems;
	  __OWL TIPtrArray<TBmpMenuImageItem*>*	ImageItems;
};
