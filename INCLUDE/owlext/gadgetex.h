...
class OWLEXTCLASS TInvisibleGadgetEx : public __OWL TGadget {
  public:
    TInvisibleGadgetEx();
...
class OWLEXTCLASS TButtonGadgetEx : public __OWL TButtonGadget
{
  public:
    TButtonGadgetEx(__OWL TResId glyphResIdOrIndex,
                    int    id,
                    TType  type = Command,
  ...
  protected:
    ...
    virtual void PaintBorder(__OWL TDC& dc);

    virtual void MouseEnter(uint modKeys, __OWL TPoint& point);


    virtual void Activate(__OWL TPoint& pt);
...
class OWLEXTCLASS TButtonTextGadgetEx : public TButtonGadgetEx
{
  public:
    TButtonTextGadgetEx(
      ...
      __OWL TResId       glyphResIdOrIndex,    // see TButtonGadgetEx
    ...
    owl_string       GetCommandText() { return CommandText; }
    virtual void SetCommandText(const owl_string& commandText);
  ...
  protected:
    ...
    virtual void Paint(__OWL TDC& dc);
    virtual void PaintText(__OWL TDC& dc);

    virtual void TryToLoadCommandText();

    virtual __OWL TDib* GetGlyphDib();

  protected:
    owl_string       CommandText;
...
class OWLEXTCLASS TPopupButtonGadgetEx : public TButtonTextGadgetEx
{
  ...
  protected:
    ...
    //
    virtual void  BeginPressed(__OWL TPoint& p);

    // Invoked by mouse exit events. sets member data "Pressed" to false and
    // paints the button in its current state
    //
    virtual void  CancelPressed(__OWL TPoint& p, bool mstate=false);

    // Captures the mouse if "TrackMouse" is set.
    //
    virtual void  LButtonDown(uint modKeys, __OWL TPoint& point);

    // Releases the mouse capture if "TrackMouse" is set.
    //
    virtual void  LButtonUp(uint modKeys, __OWL TPoint& point);
    ...
    //
    virtual void PaintButton(__OWL TDC& dc);

    // Paint the separator between button and down arrow
    //
    virtual void PaintSeparator(__OWL TDC& dc);

    // Paint the down arrow
    //
    virtual void PaintDownArrow(__OWL TDC& dc);
...
class OWLEXTCLASS TMenuButtonGadgetEx : public TPopupButtonGadgetEx
{
  public:
    TMenuButtonGadgetEx(
      ...
      __OWL TWindow*     cmdTarget,            // commando target window
      LPCTSTR      commandText,          // see TButtonTextGadget
      TDisplayType disptype,             // see TButtonTextGadget
      __OWL TResId       glyphResIdOrIndex,    // see TButtonGadgetEx
      int          id,                   // see TButtonGadgetEx
    ...
    //
    static void SetTrackPopupMenuFlags(uint flags) { Flags = flags; }
    static uint GetTrackPopupMenuFlags() { return Flags; }
...
{
  public:
    static __OWL TRect   DefaultFillRect;

  public:
    TColorButtonGadgetEx(
      TPopupType   popupType,            // see TPopupButtonGadget
      const TColorPickerData& data,      // see TColorPicker
      __OWL TColor       startColorSel,        // start color that is selected
      __OWL TRect*       fillRect,             // rect that will be filled with color
      __OWL TWindow*     parentWindow,         // parent window of picker
      LPCTSTR      commandText,          // see TButtonTextGadget
      TDisplayType disptype,             // see TButtonTextGadget
      __OWL TResId       glyphResIdOrIndex,    // see TButtonGadgetEx
      int          id,                   // see TButtonGadgetEx
  ...
  protected:
    ...
    __OWL TColor        ColorSel;         // currect selected color
    __OWL TRect*        FillRect;         // rect that will be filled with color
...
class OWLEXTCLASS TControlGadgetEx : public __OWL TControlGadget {// public TGadget {
  public:
    TControlGadgetEx(__OWL TWindow& control, TBorderStyle style = None);
...
inline __OWL TResId TButtonGadgetEx::GetResId() const {
  return ResId;
}
