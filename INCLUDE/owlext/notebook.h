...
class OWLEXTCLASS TCustomScroll {
  public:
    TCustomScroll(__OWL TWindow *Parent, int x, int y, int width, int height,
    ...
    __OWL TWindow* Parent;		// parent window of this sb
    ...
    __OWL TRect	framerect;		// rect for 3d frame
    __OWL TRect sbrect;		// rect for actual scrollbar
    ...
    //------------------------------------------
    __OWL TBrush*	framebrush;
    __OWL TBrush*	trackbrush;
    __OWL TBrush*	hilitebrush;
    __OWL TBrush*	shadowbrush;
    __OWL TBrush*	facebrush;
...
class OWLEXTCLASS THScroll: public TCustomScroll {
  public:
    THScroll(__OWL TWindow *Parent, int x, int y, int width, int height,
    ...
    void ChangeRect(const __OWL TRect& newrect, bool repaint = true);
    //----- use below 3 funcs to pass along mouse events --------
    bool MyLButtonDown(uint hitTest, __OWL TPoint& winpt);
    bool MyMouseMove(uint hitTest, __OWL TPoint& winpt);
    bool MyLButtonUp(uint hitTest, __OWL TPoint& winpt);
  ...
  protected:
    //------------------------------------------
    int 	GetMouseLoc(__OWL TPoint& winpt);
    void 	AutoScrollAction();
    void 	MyPaint(int newstate);
    void  DrawFrames(__OWL TDC& mydc);
    void 	DrawLeftArrow(__OWL TDC& mydc, bool isdown);
    void 	DrawRightArrow(__OWL TDC& mydc, bool isdown);
    void 	DrawLeftTrack(__OWL TDC& mydc, bool isdown);
    void 	DrawRightTrack(__OWL TDC& mydc, bool isdown);
    void 	DrawThumb(__OWL TDC& mydc, bool drawfocus, bool erasefocus);
    void 	DrawArrowButton(__OWL TDC& mydc, __OWL TRect& brect, bool isdown, bool left);
    void 	DrawShadeFrame(__OWL TDC& mydc, __OWL TRect& myrect, bool forthumb);
    void 	DrawDownFrame(__OWL TDC& mydc, __OWL TRect& myrect);
};
//----------------------------------------------------------------------------
class OWLEXTCLASS TVScroll : public TCustomScroll{
  public:
    TVScroll(__OWL TWindow *Parent, int x, int y, int width, int height,
    ...
    void 	ChangeRect(const __OWL TRect& rect, bool repaint = true);
  ...
  protected:
    ...
    virtual void AutoScrollAction();
    void MyPaint(int newstate);
    void DrawFrames(__OWL TDC& dc);
    void DrawTopArrow(__OWL TDC& dc, bool isdown);
    void DrawBottomArrow(__OWL TDC& dc, bool isdown);
    void DrawTopTrack(__OWL TDC& dc, bool isdown);
    void DrawBottomTrack(__OWL TDC& dc, bool isdown);
    void DrawThumb(__OWL TDC& dc, bool drawfocus, bool erasefocus);
    void DrawArrowButton(__OWL TDC& dc, __OWL TRect& rect, bool isdown, bool top);
    void DrawShadeFrame(__OWL TDC& dc, __OWL TRect& rect, bool forthumb);
    void DrawDownFrame(__OWL TDC& dc, __OWL TRect& rect);
};
////////////////////////////////////////////////////////////////////////////////
...
//----------------------------------------------------------------------------
class OWLEXTCLASS TNotebook : public virtual __OWL TWindow {
  public:
    //--- redefined functions ---
  ...
  protected:
    __OWL TRect nbcrect;              // current rect for notebook control area
    __OWL TRect vsbrect;              // rect for vertical scroll bar
    __OWL TRect hsbrect;              // rect for horizontal scroll bar
    __OWL TRect tsbrect;              // rect for tabs scroll bar
    __OWL TRect tabsrect;             // rect for set of tabs
    __OWL TRect clientrect;           // size of client area after our controls
    __OWL TRect clientprev;           // to detect window size changes
    __OWL TSize Frame;                // actual window frame sizes
    ...
    __OWL TPen* framepen;             // for drawing gray stuff over and over
    __OWL TPen* shadowpen;            // for drawing gray stuff over and over
    __OWL TFont* tabfont;             // font for tabs
    __OWL TBrush* bgbrush;            // white/ltgray dither pattern brush
    __OWL TBrush* framebrush;
    __OWL TBrush* hilitebrush;
    __OWL TBrush* shadowbrush;
    __OWL TBrush* facebrush;
    __OWL TBrush* trackbrush;
    __OWL TBrush* tabbrush;           // bg color of current tab
    __OWL TBrush* default_tabbrush;   // bg color of most tabs
    ...
    void EvSize(uint sizeType, __OWL TSize& newsize);
    void EvGetMinMaxInfo(MINMAXINFO & minmaxinfo);
    uint EvNCCalcSize(bool calcValidRects, NCCALCSIZE_PARAMS & calcSize);
    UINT EvNCHitTest(__OWL TPoint& screenPt);
    void EvNCPaint(HRGN rgn);
    void EvNCLButtonDown(uint hitTest, __OWL TPoint& screenPt);
    void EvNCLButtonDblClk(uint hitTest, __OWL TPoint& screenPt);
    void EvMouseMove(uint hitTest, __OWL TPoint& clientpt);
    void EvLButtonUp(uint hitTest, __OWL TPoint& clientpt);
    ...
    bool MyLButtonDown(uint hitTest, __OWL TPoint& screenPt);
    LRESULT NWHscroll(WPARAM wparam, LPARAM lparam);
    void ScrollTabs(int delta);
    void PaintNBC(int paintcontrol);
    void DrawShadeFrame(__OWL TDC& dc, __OWL TRect& rect);
    void DrawDownFrame(__OWL TDC& dc, __OWL TRect& rect);
    void DrawTabs(__OWL TDC& dc);
    int DrawHorzTab(__OWL TDC& dc, int tabnum, int rgnnum, int xoff);
    int DrawVertTab(__OWL TDC& dc, int tabnum, int rgnnum, int xoff);
    void EraseLastHorz(__OWL TDC& dc, int toff);
    void EraseLastVert(__OWL TDC& dc, int toff);
    void DrawBackground(__OWL TDC& dc, __OWL TRect& rect, int paintcontrol);
    void DrawTabLines(__OWL TDC& dc, __OWL TRect& rect, bool shadowline);
    void BuildBgBrush();
    //------------------------------------------
