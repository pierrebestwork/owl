...
TAppDescriptor::TAppDescriptor(TRegList& regInfo, TComponentFactory callback,
                               owl_string& cmdLine, TModule* module,
...
//
TUnknown*
TAppDescriptor::CreateAutoObject(const void* obj, const __OWL_TYPEINFO& objInfo,
...
TAppDescriptor::GetRegLink(const GUID & clsid)
{
  ...
  for (const TRegLink* link = LinkHead; link; link=link->GetNext()) {
    ...
    if (regList["progid"]) {
      ...
      if (!id) {
        if (clsid == GetLinkGuid(linkGuidIndex++))
...
TAppDescriptor::RegisterServer(TLangId lang, LPCTSTR regFilename)
{
  try {
    ...
    if (AppProgId) {       // don't attempt register app if no progid reg key
      if (!forceReg) {
        _tstrstream vstrm;
        NS_OCF::OcRegisterClass(RegInfo, Module, vstrm, lang, _T("\001\002\006"));
        if (NS_OCF::OcRegistryValidate(vstrm) != 0)
          forceReg = true;
      }
      if (forceReg) {
        _TCHAR* debugGuid = 0;
        if (DebugGuidOffset) {                // setup reg item for debug reg
          ...
          debugGuid = guidStr;
          if (NS_OCF::OcSetupDebugReg(RegInfo, regDebug, lang, debugGuid)) {
            NS_OCF::OcRegisterClass(RegInfo, Module, *strm, lang, 0, 0, OcRegNoDebug);
            NS_OCF::OcRegisterClass(RegInfo, Module, *strm, lang, AppDebugFilter,
                                                                    0, regDebug);
          }
          else {
        ...
        if (!debugGuid)
          NS_OCF::OcRegisterClass(RegInfo, Module, *strm, lang, 0, 0,
    ...
    for (const TRegLink* link = LinkHead; link; link=link->GetNext()) {
      ...
      if (regList["progid"]) {
        ...
        int debugStat = (Options & amExeModule) ?
                        NS_OCF::OcSetupDebugReg(regList, regDebug, lang, guidStr) : 0;
        ...
        if (!alwaysReg) {
          _tstrstream checkStrm;
          NS_OCF::OcRegisterClass(regList, Module, checkStrm, lang,
                            _T("\001\002\006"), 0, clsInfo);
          if (NS_OCF::OcRegistryValidate(checkStrm) == 0)
            continue;
        }
        if (debugStat) {
          NS_OCF::OcRegisterClass(regList, Module, *strm, lang, 0, regAppName, regClsid);
          if (debugStat == 1) {   // needs generated guid string
            TClassId linkClsid(GetLinkGuid(linkGuidIndex++));
            _tcscpy(guidStr, linkClsid);
          }
          NS_OCF::OcRegisterClass(regList, Module, *strm, lang, DocDebugFilter,
                                                        regAppName, regDebug);
        }
        else {
          NS_OCF::OcRegisterClass(regList, Module, *strm, lang, 0, regAppName, clsInfo);
        }
        forceReg = true;
      }
    }
...
//
void
TAppDescriptor::ProcessCmdLine(owl_string& cmdLine)
{
  struct {
    uint32 Flag;
    TCHAR* String;
    ...
    switch (cmd.Kind) {
      ...
      case TCmdLine::Option: {
        ...
        for (i = 0; i < optionTblCount; i++) {
          if (_tcsnicmp(cmd.Token, optionTbl[i].String, cmd.TokenLen) == 0) {
            Options |= optionTbl[i].Flag;
            cmd.NextToken(true);    // eat token & get next
            if (cmd.Kind == TCmdLine::Value) {
              owl_string optionArg(cmd.Token, 0, cmd.TokenLen);
