...
TOcApp::Init()
{
  ...
  for (uint i = IDS_CFFIRST; i <= IDS_CFLAST; i++) {
    TCHAR name[128];
	  if(gModule->LoadString(i, name, 128)){
...
bool
TOcApp::UnregisterClass(const owl_string& className)
{
  if (BServiceI) 
...
bool
TOcApp::RegisterClass(const owl_string& className, BCID classId, bool multiUse)
{
...
HRESULT _IFUNC
TOcApp::ComponentCreate(IUnknown * * retIface, IUnknown * outer, BCID idClass)
{
  return Registrar.OcClassMgr->ComponentCreate(retIface, outer, idClass);
}

HRESULT _IFUNC
TOcApp::ComponentInfoGet(IUnknown * * info, IUnknown * outer, BCID idClass)
{
...
TOcRegistrar::TOcRegistrar(TRegList& regInfo, TComponentFactory callback,
                           owl_string& cmdLine, TRegLink* linkHead,
                           TModule* module)
:
  TRegistrar(*new TAppDescriptor(regInfo, callback, cmdLine, module, linkHead)),
  BOleInstance(0),
  BCmI(0),
  OcClassMgr(0),
  AppCount(0)
...
typedef __export HRESULT  ( PASCAL *TCreateClassMgr)(IUnknown * *,
                                               IUnknown *, IMalloc *);
#else
typedef HRESULT PASCAL  __export (*TCreateClassMgr)(IUnknown * *,
                                               IUnknown *, IMalloc *);
#endif
