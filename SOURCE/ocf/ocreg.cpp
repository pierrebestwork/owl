...
//
long OcRegisterClass(TRegList& regInfo, TModule* module, _tostream& out,
										 TLangId lang, LPTSTR filter, TRegItem* defaults,
										 TRegItem* overrides)
{
...
_TCHAR* OcReplaceKeys[] =  { _T("progid"), _T("clsid"), _T("description"), _T("extraopt")};
_TCHAR* OcDebugKeys[] = {_T("debugprogid"), _T("debugclsid"), _T("debugdesc"), _T("debugopt")};
...
{
  LPTSTR* oldKey = OcReplaceKeys;
  LPTSTR* newKey = OcDebugKeys;
...
STDAPI __export 
#endif
DllGetClassObject(const GUID & clsid, const GUID & iid,
                                  void * * retObj)
{
  try {
    TRegistrar* registrar = 0;
...
#endif
{
  ...
  switch (reason) {
    case DLL_PROCESS_ATTACH:
      ...
      // you call OwlInitUserDLL to allow OWL to initialize 'gModule'
