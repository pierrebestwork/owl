...
TOcRemView::Init(LPCOLESTR path)
{
  ...
  //
  OcDocument.SetName(owl_string(OleStr(path)));
  return HR_NOERROR;
}
  ...
  if (open) {
    ...
    if (State == InPlaceActive) {
      ...
      if (RegList && Kind != Link) {
        owl_string newTitle = (*RegList)["menuname"];
        newTitle += gModule->LoadString(IDS_IN);
        ...
        ServerHost->EvOcViewSetTitle(NULL); // if link, caption is filename
      }
    }
  }
  else {
