...
#if defined(BI_NEED_OLEAUTO_DLL)
static _TCHAR OleAutoDllName[] = _T("OLEAUT32.DLL");
static const char SafeArrayGetVartypeStr[] = "SafeArrayGetVartype";

TModule&
TOleAutoDll::GetModule()
{
  static TModule oleAutoModule(OleAutoDllName, true, true, false);
...
{
  static TModuleProc2<HRESULT,SAFEARRAY *,VARTYPE *>
         safeArrayGetVartype(GetModule(), SafeArrayGetVartypeStr);
  return safeArrayGetVartype(psa,pvt);
}
#endif //#if defined(BI_NEED_OLEAUTO_DLL)
