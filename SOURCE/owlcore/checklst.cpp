...
//-----------------------------------------------------------------------------
int
TCheckListData::AddString(LPCTSTR str, bool isSelected)
{
...
//-----------------------------------------------------------------------------
int
TCheckListData::AddStringItem(LPCTSTR str, uint32 itemData, bool isSelected)
{
...
//-----------------------------------------------------------------------------
void    
TCheckListData::SelectString(LPCTSTR str)
{
  // temporary
  TTmpBuffer<_TCHAR> buffer(MAX_PATH);
  int index = -1;
  for(int i = 0; i < (int)Items.size(); i++){
    Items[i]->GetText(buffer,MAX_PATH);
    if(_tcscmp(buffer,str)==0){
      index = i;
      break;
    }
  }
...
TCheckList::PaintItem(DRAWITEMSTRUCT & drawInfo)
{
  ...
  //
  textRect.left++;
  PaintText(dc, textRect, item->Text.c_str());
  textRect.left--;
...
//
void
TTabCheckList::PaintText(TDC& dc, const TRect& textRect, LPCTSTR text)
{
  dc.ExtTextOut(textRect.TopLeft(), ETO_OPAQUE, &textRect, _T(""), 0, 0);
  dc.TabbedTextOut(textRect.TopLeft(),text,-1,TabStops.Size(), &TabStops[0],
                   textRect.left);
}
...
{
  LOGFONT logFont;
  (TFont(GetWindowFont())).GetObject(logFont);
...
//
void
TCheckListItem::GetText(LPTSTR buffer, int len)
{
  _tcsncpy(buffer, Text.c_str(), len-1);
  buffer[len-1] = _T('\0');
}
