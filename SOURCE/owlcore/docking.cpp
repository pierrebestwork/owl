...
void
TDockableGadgetWindow::EvOwlWindowDocked(uint loc, TDockingSlip*)
{
...
//
void
TDockableControlBar::EvOwlWindowDocked(uint pos, TDockingSlip* slip)
{
  AdjustMargins();
...
//
void
TFloatingSlip::EvWindowPosChanged(WINDOWPOS & windowPos)
{
...
//
void
TFloatingSlip::EvWinIniChange(_TCHAR * /*section*/)
...
//!CQ will return misleading Size() if dockables not 0 justified!
//
TSize
...
{
#if defined(BI_MULTI_THREAD_RTL)
  static TTlsContainer<TDitherBrush> __DitherBrush;
#else
  static TDitherBrush __DitherBrush;
#endif
  return __DitherBrush;
};
...
THarbor::EvMouseMove(uint modKeys, TPoint& point)
{
  if (DragDockable && DragDC) {
    ...
    //! horz & vert?
    //!
//!    if (!DragFrame.Contains(point))
//!      DragFrame.Offset(point.x - DragFrame.left - 15, point.y - DragFrame.top - 15);
