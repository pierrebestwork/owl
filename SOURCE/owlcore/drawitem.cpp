...
TButtonTextDrawItem::Measure(MEASUREITEMSTRUCT & measureItem)
{
  ...
  measureItem.itemHeight  = max(textSize.cy,btnSize.cy);
}
//
//
//
...
TButtonTextDrawItem::GetTextSize(TSize& size)
{
  ...
  dc.GetTextExtent(GetText(), ::_tcslen(GetText()), size);
 ...
 TDrawItemProxy(CreateProxyMenuFont())
{
  ...
  TCelArray* celAray = new TCelArray(&barRes.GetBitmap(),barRes.GetCount(),
              TSize(barRes.GetWidth(), barRes.GetHeight()), numRows);
  SetCelArray(celAray);
...
//
TDrawItem* 
TDrawMenuItemProxy::CreateItem(uint cmdId, LPCTSTR text)
{
  ...
  for(int i = 0; i < (int)items.Size(); i++){
    ...
    if(item->GetIndex() == index && _tcscmp(item->GetText(),text)==0)
      return items[i];
  }
...
//
// Returns T if a shortcut was pressed that fits into the menu and F if not.
//
bool 
TDrawMenuItemProxy::EvMenuChar(uint uChar, uint, HMENU hMenu, TParam2& param)
{
  PRECONDITION(hMenu);
  _TCHAR keyPressed = (_TCHAR)_totupper(uChar);
    ...
    if(itemId){
      ...
      if(::GetMenuItemInfo(hMenu, count, true, &itemInfo)){
        if(itemInfo.fType & MFT_OWNERDRAW){
          ...
          if(item){
            const _TCHAR* p = _tcschr(item->GetText(),_T('&'));
            if(p){
              p++;
...
TDrawItem* TDrawMenuItemProxy::FindItem(uint cmdId)
{
  ...
  return index < (size_t)items.Size() ? items[index] : 0; //JJH changed int -> size_t
}
//
