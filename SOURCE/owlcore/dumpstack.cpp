...
static const _TCHAR imgHlpStr[]                = _T("imagehlp.dll");
...
# define TRACE_STACK(m)\
    do{\
      if(__OwlDiagGroupDef.IsEnabled()){\
        diag_stream  out;\
        out << m << ends;\
        __OwlDiagGroupDef.Trace(out.str().c_str(),__FILE__,__LINE__);\
      }\
    }while(0)
#else
# define TRACE_STACK(m)\
    do{\
      if(__OwlDiagGroupDef.IsEnabled()){\
        diag_stream  out;\
        out << m << ends;\
        __OwlDiagGroupDef.Trace(out.str(),__FILE__,__LINE__);\
        out.rdbuf()->freeze(0);\
      }\
    }while(0)
#endif
...
#endif // #if !defined(__BORLANDC__) || (__BORLANDC__ >= 0x520)

//-----------------------------------------------------------------------------
#ifdef USE_MEMCHECK
#if !defined(BI_OWL_NO_DEBUG_CRT)

#if defined(BI_COMP_MSC)
#define _DEBUG
#include <crtdbg.h>
static _CRT_DUMP_CLIENT pfnOldCrtDumpClient = 0;
static _CRT_REPORT_HOOK pfnOldCrtReportHook = 0; 
//-----------------------------------------------------------------------------
static void dumpCrtDumpClient(void * pvData, unsigned int nBytes, char* buffer)
{
  try{
    // with vtable, verify that object and vtable are valid
    if(pvData==0 || IsBadReadPtr(*(void**)pvData, sizeof(void*))){
      // short form for invalid objects
      wsprintfA(buffer, "an invalid object at $%08lX, %u bytes long\n",
        pvData, nBytes);
    }
    else{
      // long form
      const type_info* ptr_info = (const type_info*)__RTtypeid(pvData);
      if(ptr_info){
        const type_info& typeinfo = *ptr_info;
        wsprintfA(buffer, "an object: %s\n",typeinfo.name());
      }
      else
        wsprintfA(buffer, "faulted while dumping object at $%08lX, %u bytes long\n",
                  pvData, nBytes);
    }
    OwlOutputDebugString(buffer);  // just for now
  }
  catch(...){
...
//-----------------------------------------------------------------------------
void __cdecl _OwlCrtDumpClient(void * pvData, unsigned int nBytes)
{
  char buffer[MAX_PATH];
  // use SEH (structured exception handling) to catch even GPFs
  //  that result from partially valid objects.
  __try{
    dumpCrtDumpClient(pvData, nBytes, buffer);
  }
  __except(EXCEPTION_EXECUTE_HANDLER){
    // short form for trashed objects
    wsprintfA(buffer, "faulted while dumping object at $%08lX, %u bytes long\n",
...
int __cdecl _OwlCrtReportHook(int nRptType, char *szMsg, int* pResult)
{
  ...
  // non-NULL m_pFile, so go through Dump for the message
  *pResult = FALSE;
  OwlOutputDebugString(szMsg); // just for now
  return TRUE;
}
//-----------------------------------------------------------------------------
/*struct __TMemDebug{
...
__TMemDebug::__TMemDebug()
{
  CHECK(pfnOldCrtDumpClient);
  pfnOldCrtDumpClient = _CrtSetDumpClient(_OwlCrtDumpClient);

  CHECK(pfnOldCrtReportHook);
  pfnOldCrtReportHook = _CrtSetReportHook(_OwlCrtReportHook);
...
#pragma init_seg(lib)
//static __TMemDebug __owlDebugState;

//-----------------------------------------------------------------------------
#endif // BI_COMP_MSC
#endif // !defined(BI_OWL_NO_DEBUG_CRT)
#endif // USE_MEMCHECK
//-----------------------------------------------------------------------------
