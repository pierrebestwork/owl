...
TGadgetWindow::EnableFlatStyle(uint style)
{
  if(style == FlatDefault)
  {
    ...
    // The following may throw if v > 6 and the theme module is unavailable.
    // Probably this never happens, but if it does we fallback to the old style.
    //
    try
    {
      if (v >= 0x60000 && TThemeModule::GetInstance().IsAppThemed())
      style |= FlatXPTheme;
  }
    catch (const TXOwl&)
    {}
...
{
  return (FlatStyle & FlatXPTheme) && TThemeModule::GetInstance().IsThemeActive();
}
...
TGadgetWindow::LayoutHorizontally(TLayoutInfo& layout)
{
  ...
  if (WideAsPossible) {
    ...
    wideAsPossibleWidth = max(( (innerRect.Width() - width) / (int)WideAsPossible ), 0);
  }
  else
  ...
  for (gadget = Gadgets; gadget; gadget = gadget->Next, i++) {
    ...
    if (gadget->WideAsPossible)
      bounds.right = bounds.left + __max(wideAsPossibleWidth, desiredSize.cx);
    else
    ...
    //
    layout.DesiredSize.cx = __max(int(bounds.right+rightM), layout.DesiredSize.cx);
    layout.DesiredSize.cy = __max(int(bounds.bottom+bottomM), layout.DesiredSize.cy);
  }
}
...
TGadgetWindow::LayoutVertically(TLayoutInfo& layout)
{
  ...
  for (gadget = Gadgets; gadget; gadget = gadget->Next, i++) {
    ...
    //
    // SIR June 20th 2007 max instead of ::max
    layout.DesiredSize.cx = (int)max(int(bounds.right+rightM), (int)layout.DesiredSize.cx);
    layout.DesiredSize.cy = (int)max(int(bounds.bottom+bottomM), (int)layout.DesiredSize.cy);
  }
}
...
//!CQ  memset((void*)layout.GadgetBounds, 0, sizeof(TRect) * NumGadgets);// Debugging code
  ...
  for (gadget =  Gadgets; gadget; gadget = gadget->Next, i++) {
    ...
    if ((bounds.right > right || contBreak) &&
      ...
      layout.DesiredSize.cx =
        __max((layout.GadgetBounds[iend].right+rightM), layout.DesiredSize.cx);
  ...
  // SIR June 20th 2007 max instead of ::max
  layout.DesiredSize.cx =
    (int)max(int(layout.GadgetBounds[iend].right+rightM), (int)layout.DesiredSize.cx);
...
TGadgetWindow::CleanupWindow()
{
  ...
  if (AtMouse)
  {
    TPoint  TemporaryObject(-1, -1);// << Peter Sliepenbeek: GCC needs temporary, SL: CBuilder 2011 also.
    AtMouse->MouseLeave(0, TemporaryObject);
...
TGadgetWindow::EvLButtonUp(uint modKeys, TPoint& point)
{
  ...
  if (gadget && (gadget->GetEnabled() || Capture)){
    TPoint pt (point.x - gadget->GetBounds().left, 
               point.y - gadget->GetBounds().top);
    gadget->LButtonUp(modKeys, pt);
  }
...
TGadgetWindow::EvMouseMove(uint modKeys, TPoint& point)
{
  struct Local 
  { 
    typedef void (TGadget::*MouseFun)(uint, TPoint&);
