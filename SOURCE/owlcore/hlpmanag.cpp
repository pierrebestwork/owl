...
static _TCHAR HtmlHelpDllName[] = _T("HHCTRL.OCX");
...
// context table.
//
THelpFileManager::THelpFileManager(const owl_string& helpFileName)
:
#if !defined(NO_HTMLHELP)
  UseHTMLHelp(false),
  WinToHTML(true),
...
//
void
THelpFileManager::SetHelpFile(const owl_string& helpFileName)
{
...
THelpFileManager::EvHelp(HELPINFO* hi)
{
  ...
  if (::GetKeyState(VK_F1) < 0 && ::GetKeyState(VK_SHIFT) < 0)
  {
    ...
    LastHit = TPoint(wnd->Attr.X + child->Attr.X, wnd->Attr.Y + child->Attr.Y);
  }
