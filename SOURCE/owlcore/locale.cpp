...
#endif
{
 ...
 ~_TLocaleCacheList();                        // releases cache entries
  ...
  _TLocaleCache* Lookup(const _TCHAR* name); // returns cache entry, 0 if failed
...
{
#if defined(BI_MULTI_THREAD_RTL)
  static TTlsContainer<_TLocaleCacheList> __LocaleCacheList;
#else
  static _TLocaleCacheList __LocaleCacheList;
#endif
  return __LocaleCacheList;
};
...
struct _TLocaleCacheBase {
  ...
  _TLocaleCacheBase(const _TCHAR* name, long hash);
};
...
#endif
  _TLocaleCache(const _TCHAR* name, long hash, HRSRC rscHdl, HGLOBAL resData);
 ~_TLocaleCache() {}
...
_TLocaleCacheBase::_TLocaleCacheBase(const _TCHAR* name, long hash)
:
  Hash(hash),
  Neutral(name),
  Next(0)
...
_TLocaleCache::_TLocaleCache(const _TCHAR* name, long hash,
...
//
//
_TLocaleCache* _TLocaleCacheList::Lookup(const _TCHAR* name)
{
  LOCKCASHE
  const _TCHAR* neut = name + 1;      // skip over prefix flag _TCHAR
  long hash = 0;
...
int TLocaleString::Compare(const _TCHAR * str, TLangId lang)
{
