...
//
TPalette::TPalette(const _TCHAR * fileName)
{
...
//
bool
TPalette::Read(const _TCHAR * filename)
{
  TRiffFile file(filename);
  if (!file.IsOpen())
    return false;

  const _TCHAR* lp = _tcsrchr(filename, _T('.'));
  if(lp){
...
//
bool
TPalette::Write(const _TCHAR * filename)
{
...
//@mfunc write Microsoft *.pal format
//@parm HMMIO | hmmio | file handle
...
# pragma warn .ccc
  ...
  TPointer<char> pdata = new char[iSize];
...
/* -------------------------------------------------------------------------- */
//@func HSL_to_RGB -> convert HSL to RGB
// given h,s,l on [0..1],return r,g,b on [0..1]
//@parm double& | h |  H component
//@parm double& | sl | Sl component
//@parm double& | l  | L component
//@parm double* | r  | R component
//@parm double* | g  | G component
//@parm double* | b  | B component
...
#include <pshpack1.h>
//@struct ACO_Hdr | *.ACO header
struct ACO_Hdr{
  uint16 Version; //@field Version== 1
  uint16 Count;   //@field number of color
};
...
//@struct ACO_Color | *.ACO color structure
struct ACO_Color{
  uint16 clrSpace;  //@field  color space
  uint16 Color1;     //@field  color value1
  uint16 Color2;    //@field  color value2
  uint16 Color3;    //@field  color value3
  uint16 Color4;    //@field  color value4
};
...
/* -------------------------------------------------------------------------- */
//@mfunc Read Adobe Photoshop *.ACO color table
//@parm HMMIO | hmmio | file handle
//@rdesc true if success
...
/* -------------------------------------------------------------------------- */
//@mfunc Write Adobe Photoshop *.ACO color table
//@parm HMMIO | hmmio | file handle
//@rdesc true if success
...
// -----------------------------------------------------------------------------
//@struct ACT_Color | *.ACT color structure
struct ACT_Color{
  uint8 Red;    //@field Red compponent
  uint8 Green;  //@field Green compponent
  uint8 Blue;   //@field Blue compponent
};
// -----------------------------------------------------------------------------
//@mfunc Read Adobe Photoshop *.ACT color table
//@parm HMMIO | hmmio | file handle
//@rdesc true if success
...
/* -------------------------------------------------------------------------- */
//@mfunc Read color table from *.DIB
//@parm HMMIO | hmmio | file handle
//@rdesc true if success
...
/* -------------------------------------------------------------------------- */
//@mfunc Write Color table to *.DIB with dimensions 1x1 pixel
//@parm HMMIO | hmmio | file handle
//@rdesc true if success
...
//
//
TPalette::TPalette(const BITMAPCOREINFO * core, uint flags)
{
  Create(core, flags);
}
...
//
void
TPalette::Create(const BITMAPCOREINFO * coreInfo, uint flags)
{
  const RGBTRIPLE * rgb = coreInfo->bmciColors;
