...
// HP doesnt do namespaces but Solaris needs this to avoid std::'ing every list here
#ifdef __SUNPRO_CC
...
TVSplitter::Setup(TWindow* targetPane, TWindow* newPane, float percent)
{
  ...
  // of target pane.
  //
  lmOfPane1.Width.Absolute(static_cast<int>(targetPane->Attr.W * percent));
...
THSplitter::Setup(TWindow* targetPane, TWindow* newPane, float percent)
{
  ...
  // target pane.
  //
  lmOfPane1.Height.Absolute(static_cast<int>(targetPane->Attr.H * percent)); //JJH added static cast
