...
TPXPictureValidator::Error(TWindow* owner)
{
  ...
  owl_string msgTmpl = app->LoadString(IDS_VALPXPCONFORM);
  TAPointer<TCHAR> msg = new TCHAR[msgTmpl.length() + Pic.length() + 1];
  _stprintf(msg, msgTmpl.c_str(), (LPCTSTR)Pic.c_str());
  // if caption == 0 -> set it to Name of programm
...
//
int
TPXPictureValidator::Adjust(owl_string& /*text*/, uint& /*begPos*/,
    ...
    switch (Pic[i]) {
      ...
      case _T('!'): {
        ...
        if (j + n >= len)
          ...
          if (n == 1)
            input[j++] = (_TCHAR)_totupper((TCHAR)ch);
          else
            j += n;
        }
#else
        input[j++] = (_TCHAR)_totupper(ch);
#endif
        i++;
        break;
      }
      ...
      default: {
        ...
        if (n2 == 2) {
          ...
          if (uc1 == 0x81 && uc2 == 0x93) {
            if ((uchar)ch >= 0xA1 && (uchar)ch <= 0xDF){
              i += n2;
              j += n;
              break;
            }
            ...
            if (n == 2 && j + n < len &&
                ((uchar)ch != 0x81 || (uchar)input[j+1] != 0x40)) {
              i += n2;
              j += n;
              break;
            } else
...
//
            ...
            if (n != n2)
              return prError;
            if (_totupper((_TCHAR)Pic[i]) != _totupper((_TCHAR)ch))
              return prError;
            input[j++] = Pic[i++];
          }
        }
