           ...
           , NewResource(0)
{
  ...
  //
  NewResource = new TResource<DLGTEMPLATEEX, (long)RT_NEWDIALOG>(hInst, id);
  if (!NewResource->IsOK()) 
    // When the new resource cannot be found in 32-bit, try
    // the old DLGTEMPLATE resource
    //
    OldResource = new TResource<DLGTEMPLATE, (long)RT_DIALOG>(hInst, id);
}
...
TDialogRes::TDialogRes(HINSTANCE hInst, TResId id, LANGID langid)
          ...
          , NewResource(0)
{
  NewResource = new TResource<DLGTEMPLATEEX, (long)RT_NEWDIALOG>(hInst, id, langid);
  if (!NewResource->IsOK())
    OldResource = new TResource<DLGTEMPLATE, (long)RT_DIALOG>(hInst, id, langid);
}
...
:
  TResource<TOOLBARTEMPLATE, (long)RT_TOOLBAR>(hInst, id), 
  ToolbarBitmap(0)
{
