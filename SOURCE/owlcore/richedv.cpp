...
{
  TPointer<_tistream> inStream = Doc->InStream(ios::in | ios::binary);
  if (!inStream) {
...
TRichEditView::VnCommit(bool force)
{
  ...
  TPointer<_tostream> outStream = Doc->OutStream(ios::out | ios::binary);
  if(!outStream){
    Doc->PostError(IDS_UNABLEOPEN, MB_OK);
    return false;
  }
...
TRichEditView::CmParagraphBullet()
{
  ...
  pFormat.wNumbering = (pFormat.wNumbering == 0) ? (WORD)PFN_BULLET : (WORD)0;
