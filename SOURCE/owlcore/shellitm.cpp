...
// delay loading SHELL32.DLL/SHELL.DLL
  static const _TCHAR shellStr[]            = _T("SHELL32.DLL");
...
// Expanded by Val Ovechkin 11:57 AM 6/3/98
    ...
    newPidl = REINTERPRET_CAST(ITEMIDLIST*, p);
...
//
TShellItem::TShellItem(const _TCHAR* path, bool throwOnInvalidPath, HWND windowOwner)
...
//          REINTERPRET_CAST(void **, STATIC_CAST(IShellFolder**, cs.ParentFolder)));
    ...
    WARNX(OwlShell, FAILED(hr), 0, _T("IShellFolder::BindToObject failed.  TShellItem* = ")
          << hex << (LPVOID)this << _T("  Parent ITEMIDLIST* = ")
          << STATIC_CAST(ITEMIDLIST*, (ITEMIDLIST*)newFullPidl.StripLastItem()));
    TXShell::Check(hr, IDS_BINDTOOBJECTFAIL);
  }
...
// Expanded by Val Ovechkin 12:24 PM 6/3/98
  void *p = STATIC_CAST(ITEMIDLIST*, *CONST_CAST(TShellItem*, this));
  DWORD rc = ::SHGetFileInfo(
    REINTERPRET_CAST(const _TCHAR*, p),
    0, &fi, sizeof (fi), SHGFI_TYPENAME | SHGFI_PIDL);

//  DWORD rc = TShell::SHGetFileInfo(
//    REINTERPRET_CAST(const _TCHAR*, STATIC_CAST(ITEMIDLIST*, *CONST_CAST(TShellItem*, this))),
//    0, &fi, sizeof (fi), SHGFI_TYPENAME | SHGFI_PIDL);
...
TShellItem::GetDisplayName(const TDisplayNameKind kind) const
{
  ...
  switch (strRet.uType) {
    ...
    case STRRET_OFFSET: // Offset into the piddle
      s = REINTERPRET_CAST(char*, STATIC_CAST(ITEMIDLIST*, (ITEMIDLIST*)Pidl)) + strRet.uOffset;
      break;
    }
...
//
void
TShellItem::Rename(const _TCHAR *newName, HWND windowOwner, const TDisplayNameKind kind)
{
  LPITEMIDLIST pidl;
  ...
  TShell::SHChangeNotify(isFolder? SHCNE_RENAMEFOLDER: SHCNE_RENAMEITEM,
                   SHCNF_PATH /*| SHCNF_FLUSH*/, STATIC_CAST(const _TCHAR*, oldPath),
                   newPath);
}
...
//
bool 
TShellItem::Copy(const _TCHAR* dest, const bool destIsFolder,
                 const USHORT flags, const _TCHAR* title,
                 HWND windowOwner) const
{
  ...
  fo.hwnd = windowOwner;
  fo.wFunc = FO_COPY;
  _TCHAR srcPath[_MAX_PATH];
  ::memset(srcPath, 0, sizeof (srcPath));
  ::_tcscpy(srcPath, GetDisplayName(ForParsing));
  fo.pFrom = srcPath;
  _TCHAR destPath[_MAX_PATH];
  ...
  TShell::SHChangeNotify(isFolder? SHCNE_MKDIR: SHCNE_CREATE, SHCNF_PATH /*| SHCNF_FLUSH*/,
                   STATIC_CAST(const _TCHAR*, GetDisplayName(ForParsing)), 0);
  return true;
}
...
//
bool 
TShellItem::Copy(const TShellItem& dest, const USHORT flags, const _TCHAR* title,
                 HWND windowOwner) const
{
...
//
bool
TShellItem::Move(const _TCHAR* destFolder, const USHORT flags, const _TCHAR* title,
                 HWND windowOwner)
{
  EnforceValidity();
  SHFILEOPSTRUCT fo;
  ...
  *this = TShellItem(newPath);              // change myself to the new location
  ...
  TShell::SHChangeNotify(isFolder? SHCNE_RMDIR: SHCNE_DELETE, SHCNF_PATH /*| SHCNF_FLUSH*/,
                   STATIC_CAST(const _TCHAR*, oldPath), 0);
...
TShellItem::Move(const TShellItem& destFolder, const USHORT flags,
                 const _TCHAR* title, HWND windowOwner)
{
...
//
bool 
TShellItem::Delete(const USHORT flags, const _TCHAR* title, HWND windowOwner)
{
  EnforceValidity();
...
TShellItem::GetIcon(TIconSize size, uint kind)
{
  ...
  TShell::SHGetFileInfo(REINTERPRET_CAST(const _TCHAR*, STATIC_CAST(ITEMIDLIST*, *this)),
...
TShellItem::BrowseForFolder(TCreateStruct& cs, HWND windowOwner,
                            const _TCHAR* title, const UINT flags, int* image,
                            const bool includeStatus, BFFCALLBACK func,
                            const LPARAM param) const
{
  EnforceValidity();
  RetrieveIShellFolder();
  _TCHAR displayName[_MAX_PATH];
...
//
HRESULT 
TShellItem::ParseDisplayName(TCreateStruct& cs, const _TCHAR* displayName,
