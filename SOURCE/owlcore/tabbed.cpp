...
IMPLEMENT_CASTABLE(TTabbedBase);
...
{
  LPCTSTR text = titles ? titles : wnd.Title;
  if(HiUint16(uint32(text))==0)
    text = _T("");

  int index = NoteTab->Add(text, uint32((TWindow*)&wnd));
