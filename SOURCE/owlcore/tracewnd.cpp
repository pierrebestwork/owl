...
//   This source file implements the TTaceWindow class.
//----------------------------------------------------------------------------
#include <owl/pch.h>
...
{
  const uint  MaxBuf = 51000u;  // slightly conservative guess
  uint        textlen = GetWindowTextLength();
  uint        szlen = ::_tcslen(sz);
  uint        bufsize;
  LPCTSTR      p;
  LPTSTR      p2;
  ...
  if (uint32(szlen) + textlen > MaxBuf)
    bufsize = MaxBuf;
  else
    bufsize = szlen + textlen;

  ++bufsize;  // account for null terminator

  // it's hard to tell just how much we can fit into the edit buffer, but
  // assume we can fit what's already there!
  if (bufsize < textlen+1)
      bufsize = textlen+1;

  TAPointer<_TCHAR> buffer = new _TCHAR [bufsize];
  if (! buffer)
    return;

  GetWindowText (buffer, bufsize);

  if (uint32(szlen) + textlen > bufsize){
    uint    removeAtLeast = uint((uint32(szlen) + textlen) - bufsize);

    if (removeAtLeast >= textlen)
      textlen = 0;
    else{
      p = buffer + removeAtLeast;
      textlen -= removeAtLeast;

      while (textlen && *p && *p != _T('\n')){ 
        ++ p; --textlen; 
      }
      if (textlen && *p==_T('\n')){ 
        ++p; --textlen; 
      ...
      // textlen is the number of characters we want to keep, and they
      // start at p

      if (textlen)
        memmove (buffer, p, textlen);
    }
  }
  ...
  *p2 = 0;
  ...
  while (uint(SendMessage(WM_GETTEXTLENGTH)) < textlen){
    while (textlen && *p && *p != _T('\n')){ 
      ++ p; -- textlen; 
    }
    if (textlen && *p==_T('\n')){ 
...
class TGroupList: public TCheckList{
  public:
    ...
    void    AddItem(LPCTSTR text, bool checked, void * data);
...
//
void 
TGroupList::EvLButtonDown(uint modKeys, TPoint& point)
{
...
//
void 
TGroupList::AddItem(LPCTSTR text, bool checked, void* data)
{
...
//
void  
TGroupList::SetItemText(int index, LPCTSTR text)
{
...
TTraceWindow::SetupWindow ()
{
  ...
  owl_string title = _T("Diagnostic Window - ");

  title += GetApplication()->GetMainWindow()->Title;
...
TTraceWindow::FormatGroup (TDiagBase* group)
{
  _TCHAR  level [10];
  _USES_CONVERSION;
  owl_string s = _A2W(group->GetName());
...
TTraceWindow::CmSave ()
{
  ...
  data.InitialDir = const_cast<_TCHAR*> (TraceDir.c_str());

  owl_string filename = TraceDir + _T("\\Trace.t00");
  int     n = 0;
  ...
  while(TFileName(filename).Exists()){  // while (file exists)
    ...
    filename [filename.length() - 1] = _TCHAR (_T('0') + (n % 10));
    filename [filename.length() - 2] = _TCHAR (_T('0') + ((n / 10) % 10));

    if (n > 99)
      filename [filename.length() - 3] = _TCHAR (_T('0') + ((n / 100) % 10));
  }
  ...
  if (IDOK == dlg.Execute()){
    uint len = Edit->GetWindowTextLength();
...
void TTraceWindow::CmSetLevel ()
{
  ...
  owl_string  prompt = _T("Set level for "); 
...
void TTraceWindow::AddModules(TModule* _module)
{
  ...
  while(module){
    int index = TraceModules->AddString(FormatLink(module).c_str());
