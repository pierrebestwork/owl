...
{
#if defined(BI_MULTI_THREAD_RTL)
  static TTlsContainer<TUIFaceData> __UIFaceData;
#else
  static TUIFaceData __UIFaceData;
#endif
  return __UIFaceData;
};
...
//
int
TUIFace::DrawTextEtched(TDC& dc, LPCTSTR str, int count,
...
//
void
TUIFace::PaintText(LPCTSTR text, TDC& dstDC, const TRect& faceRect,
  ...
  switch (state) {
    ...
    case Normal: {
      ...
      if (fillFace){
        ...
        dstDC.ExtTextOut(0, 0, ETO_OPAQUE, &faceRect, 0,0);
      }
    ...
    case Down: {
      ...
      if (fillFace){
        bkColor = dstDC.SetBkColor(fillColor);
        if(pressed)
          dstDC.ExtTextOut(dstPt, ETO_OPAQUE, &faceRect, 0,0);
      }
