...
TSocket::~TSocket()
{
  if (Handle != static_cast<unsigned int>(INVALID_SOCKET) && !SaveSocket) {
...
int TSocket::CreateSocket()
{
  ...
  return SocketsCallCheck(Handle == static_cast<unsigned int>(INVALID_SOCKET)); //JJH added static cast
}
...
{
  if (Handle != static_cast<unsigned int>(INVALID_SOCKET)) { //JJH added static cast
