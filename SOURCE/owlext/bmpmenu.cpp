...
#define BMPMENU_ARRAYLIMIT NPOS
#else
#define BMPMENU_ARRAYLIMIT INT_MAX
#endif
	...
	if (DIS.CtlType == ODT_MENU)
	{
		...
		if (pItem)
		{
			...
			if (pItem->Cmd == -3) // is a separator
			{
				...
				owl_string cs1;
				owl_string cs(pItem->Text);
				TSize sz = dc.GetTextExtent(cs.c_str(), _tcslen(cs.c_str()));
			...
			else
			{
				...
				if (cs != _T(""))
				{
					...
					else
					{
						...
						TSize sz = dc.GetTabbedTextExtent(cs.c_str(), _tcslen(cs.c_str()), 0, 0);
						MIS.itemWidth = MIS.itemHeight + 2 + sz.cx;

						// make the menu bigger, because sometimes the calculated
						// width is not enough
						//
						MIS.itemWidth += 2 * (sz.cx/_tcslen(cs.c_str()));
					}
		...
		if (itemId != 0){
			...
			if (::GetMenuItemInfo(hMenu, nItem, TRUE, &mii)){
				if (mii.fType&MFT_OWNERDRAW){
					...
					if (pItem){
						owl_string csItem(pItem->Text);
						_tcslwr((_TCHAR*)csItem.c_str());
						int iAmperIdx = csItem.find(_T("&"));
						if (iAmperIdx >= 0){
...
{
	if (ExcludedBitmaps->Find(resId) == BMPMENU_ARRAYLIMIT)
...
bool TBmpMenu::TryToLoadBmpResource(unsigned int resId)
{
	...
	//
	if (ExcludedBitmaps->Find(mapResId) != BMPMENU_ARRAYLIMIT)
		return false;
