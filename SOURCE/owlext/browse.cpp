...
struct TColumn {
	TColumn() {}
	TColumn(int id, const owl_string& text, int styleNo = 0)
...
void TBrowseList::EvRButtonDown(uint modKeys, TPoint& point)
{
	MapWindowPoints(*Parent, &point, 1);
	Parent->SendMessage(WM_RBUTTONDOWN, modKeys, MAKELPARAM(point.x, point.y));
}

void TBrowseList::EvRButtonUp(uint modKeys, TPoint& point)
{
	MapWindowPoints(*Parent, &point, 1);
...
THeader::THeader(int id, const owl_string& title, int width, bool visible,
				 bool exactWidth)
{
	Id = id;
	Title = title;
						...
						TUnits units)
{
	switch (units){
case Chars:
	width = width < 0 ? _tcslen(title) : width;
	width *= BoldCharWidth;
	break;
case Pixels:
...
int TBrowse::AddRow()
{
	...
	if (GetNumCols() && (index = List->AddString(_T(""))) != LB_ERR)
	{
		...
		List->SetItemData(index, (uint32)columns);
	}
	...
	while (i){
		...
		if (header->Visible){
			if (header->ExactWidth)
				...
				rect.right = rect.left + 4 + (header->Width / 100.0 * browseWidth);
			header->Rect = rect;
			Extent += rect.Width() - 1;
		...
		case ODA_SELECT: {
			...
			while (i){
				...
				if (header->Visible){
					...
					int loc = cols->Find(TColumn(header->Id, _T(""), 0));
					if (loc != NPOS){
						int locStyle = Styles->Find(&TStyle((*cols)[loc].StyleNo));
						if (locStyle == NPOS)
							locStyle = Styles->Find(&TStyle(header->StyleNo));
						...
						int length = _tcslen((*cols)[loc].Text.c_str());
						TSize size = dc.GetTextExtent((*cols)[loc].Text.c_str(), length);
						if (size.cx >= rect.Width())
						{
							...
							for (pch = longWord + length - 1; pch >= longWord; --pch){
								_tcscpy(pch, _T("..."));
								size = dc.GetTextExtent(longWord, _tcslen(longWord));
								if (size.cx < rect.Width())
									break;
							}
...
void TBrowse::EvLButtonDown(uint modKeys, TPoint& point)
{
	bool onBorder;
	Capture = GetHeaderFromPoint(point, onBorder);

	if (Capture != NULL){
		SetCapture();
		if (onBorder){
			ReSizing = true;
			StartDrawingSizingLine(point);
		}
...
void TBrowse::EvLButtonUp(uint modKeys, TPoint& point)
{
	if (Capture){
		if (ReSizing){
			StopDrawingSizingLine(point);
			TRect rect = Capture->Rect.OffsetBy(-ViewportOrgX, 0);
			if (point.x < rect.right - 1 || point.x > rect.right + 1){
				if (point.x < rect.left)
					point.x = rect.left;
				if (Capture->ExactWidth)
					Capture->Width = point.x - rect.left - 4;
				else
					Capture->Width = (point.x - rect.left - 4) * 100.0 / List->Attr.W;
...
void TBrowse::EvMouseMove(uint modKeys, TPoint& point)
{
	if (!Capture){
		bool onBorder;
		if (GetHeaderFromPoint(point, onBorder) != NULL && onBorder)
		...
		if (ReSizing)
			...
			bool onBorder;
			THeader* header = GetHeaderFromPoint(point, onBorder);
...
uint TBrowse::EvNCHitTest(TPoint& /*point*/)
{
	return HTCLIENT;
}

void TBrowse::EvRButtonDown(uint modKeys, TPoint& point)
{
	if (Parent){
		MapWindowPoints(*Parent, &point, 1);
		Parent->SendMessage(WM_RBUTTONDOWN, modKeys, MAKELPARAM(point.x, point.y));
...
void TBrowse::EvRButtonUp(uint modKeys, TPoint& point)
{
	if (Parent){
		MapWindowPoints(*Parent, &point, 1);
		Parent->SendMessage(WM_RBUTTONUP, modKeys, MAKELPARAM(point.x, point.y));
...
void TBrowse::EvSize(uint sizeType, TSize& size)
{
...
bool TBrowse::GetColumnText(int id, owl_string& text, int index)
{
	bool bOk = false;

	uint32 itemData = List->GetItemData(index);
	if ((long)itemData != LB_ERR){
		TColumns* cols = (TColumns*)itemData;
		int loc = cols->Find(TColumn(id, _T(""), 0));
		if (loc != NPOS){
			text = (*cols)[loc].Text;
			bOk = true;
		}
...
{
	int loc = Headers->Find(&THeader(id, _T(""), 0, true, true));
	if (loc != NPOS){
...
void TBrowse::Init()
{
	...
	NormalFont = new TFont(_T("MS Sans Serif"), -11);

	LOGFONT lf;
	NormalFont->GetObject(lf);
	lf.lfWeight = FW_BOLD;
...
void TBrowse::Paint(TDC& dc, bool /*erase*/, TRect& rect)
{
	...
	if (HIBYTE(LOWORD(GetVersion())) >= 95 || LOBYTE(LOWORD(GetVersion())) > 3) //??????????
		ViewportOrgX = List->GetScrollPos(SB_HORZ);
	else
		ViewportOrgX = (float(HorzExtent) - List->GetClientRect().Width())
		/ 100 * List->GetScrollPos(SB_HORZ);
...
bool TBrowse::SetColumnWidth(int id, int width, TUnits units)
{
	...
	if (loc != NPOS){
		...
		switch (units){
case Chars:
	width = width < 0 ? _tcslen((*Headers)[loc]->Title.c_str()) : width;
	width *= BoldCharWidth;
	break;
case Pixels:
	width = width < 0 ? 0 : width;
...
{
	int loc = Headers->Find(&THeader(colId, _T(""), 0, true, true));
...
void TBrowse::SetupWindow()
{
	...
	if (Attr.Style & TBS_MULTIPLESEL)
		List->Attr.Style |= LBS_MULTIPLESEL;
	if (Attr.Style & TBS_EXTENDEDSEL)
		List->Attr.Style |= LBS_EXTENDEDSEL;
...
void TBrowse::SortItems(int colId, bool ascending)
{
	...
	for (curRow = 0; curRow < rowCount; curRow++){
		...
		for (nextRow = curRow; nextRow < rowCount; nextRow++){
			owl_string strNextRow, strMinRow;
...
bool TRowArrayIterator::GetColumnText(int id, owl_string& text)
{
