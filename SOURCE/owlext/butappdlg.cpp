	...
	if(!CelArray){
		for (int i=0; i < GetCount(); i++) {
			TModule* module = FindResourceModule(Parent,GetModule(), 
				GetItemData(i), RT_BITMAP);
			if(module){
				TBitmap bmp(*module, TResId(GetItemData(i)));
				if(!CelArray)
...
void TImageListBox::BuildIdsArray(TResId& resId, int maxitems)
{
	...
	if(IdsArray){
		...
		if(CelArray){
			AddString(_T(""));
			SetItemData(i, (uint32)(_TCHAR*)resId);
			TModule* module = FindResourceModule(Parent,GetModule(), 
				resId, RT_BITMAP);
			if(module){
		...
		AddString(_T(""));
		SetItemData(0, (uint32)(_TCHAR*)resId);
...
void TButtonAppearanceDialog::CmOk()
{
	...
	if (_tcslen(text) > 0) 
		ButtonTextGadget->SetCommandText(owl_string(text));

	ButtonTextGadget->SetDisplayType(DisplayType);
	uint32 data = ImageListBox->GetItemData(ImageListBox->GetSelIndex());
	ButtonTextGadget->SetResId(data);
	TDialog::CmOk();
}
//
