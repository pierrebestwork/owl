...
{
	_TCHAR buf[64] ;
	_stprintf( buf, _T("Value must be between %.*f and %.*f"),       //????????????????????
		owner->GetDecimals(), min, owner->GetDecimals(), max ) ;
	owner->MessageBox( buf, _T("Error"), MB_ICONEXCLAMATION|MB_OK|MB_TASKMODAL);
}
...
void TCalc::SetDisplay(double r)
{
	...
	if (_tcslen(first) > CALC_DIGITS + 1 + 10 )
		...
		charsToCopy = min(CALC_DIGITS + 1, int(last - first));
...
void TCalcEdit::Clicked()
{
	...
	((TWindow*)pCalc)->Attr.AccelTable = IDA_CALCULATOR;
