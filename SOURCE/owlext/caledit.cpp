	...
	else
	{
		...
		if ((int)Date.DayOfMonth() <= days)
			...
			if (Date.Month() == 1)
				...
				Date += __min(days, Date.DaysInMonth());
	}
	break;
case VK_PRIOR:
	if (GetKeyState(VK_CONTROL) < 0){
...
void
TPopupCalWin::EvLButtonDown(uint modKeys, TPoint& point)
{
	SetCapture();
...
void
TPopupCalWin::EvLButtonUp(uint /*modKeys*/, TPoint& point)
{
	ReleaseCapture();
...
void
TPopupCalWin::EvMouseMove(uint modKeys, TPoint& point)
{
	if (Dragging){
...
TPopupCalWin::Paint(TDC& dc, bool /*erase*/, TRect& /*rect*/)
{
	...
	TFont boldFont(_T("MS Sans Serif"), -8, 0, 0, 0, FW_BOLD);
