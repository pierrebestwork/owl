								   ...
								   uint     customFieldResId)
{
	...
	for (uint i=0; i<NumColors; i++)
		Colors[i] = colors[i];
	NumColumn = numColumn;
	NumColumn = min(NumColumn, NumColors);
...
void TColorPicker::EvActivateApp(bool active, HTASK hTask)
{
...
//
LPCTSTR TColorPicker::GetClassName()
{
	return _T("TColorPickerWindow");
}
...
void TColorPicker::DrawCell(TDC& dc, int index)
{
	...
	if (index >= 0 && index < (int)Data.NumColors) {
		...
		if (index != INVALID_COLOR) {
			...
			int oldtextcol = dc.SetTextColor(TColor::SysBtnText);
			int oldbkmode = dc.SetBkMode(TRANSPARENT);
#if !defined(BI_STD_STRING)
			if (index == DEFAULT_BOX_VALUE && !DefaultText.is_null()) {
#else
...
#if !defined(BI_STD_STRING)
			if (index == CUSTOM_BOX_VALUE && !CustomText.is_null()) {
#else
...
//
void TColorPicker::EvMouseMove(uint modKeys, TPoint& point)
{
...
//
void TColorPicker::EvLButtonDown(uint modKeys, TPoint& point)
{
