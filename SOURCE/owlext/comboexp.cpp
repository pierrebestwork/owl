...
TExpandableComboBox::AddString(LPCTSTR str)
{
	...
	if(index >= 0){
		...
		StoreStrExtent(str);
		// now the item data...
		TComboBox::SetItemData(index, (uint32)(new TCBItemData()));
	}
	return index;
}
...
TExpandableComboBox::InsertString(LPCTSTR str, int index)
{
	...
	if(actualIndex >= 0){
		...
		StoreStrExtent(str);
		// now the item data...
		TComboBox::SetItemData(actualIndex, (uint32)(new TCBItemData()));
	}
	return actualIndex;
}
...
//----------------------------------------------------------------
int
TExpandableComboBox::SetItemData(int index, uint32 data)
{
...
{
	if (msg == WM_CTLCOLOR){
		if (HiUint16(lParam) == CTLCOLOR_LISTBOX){
			if (hwndList == 0){
				bFirstRedraw = true;
				hwndList = (HWND)LoUint16(lParam);
				::GetWindowRect(hwndList, &RectList);
				::SendMessage (hwndList, WM_HSCROLL, SB_TOP, 0);
				//       ::SetScrollRange (hwndList, SB_HORZ, 0, greatestExtent, TRUE);
			}
			ResizeToFit();
		}
	}
...
TExpandableComboBox::ResizeToFit()
{
	...
	if (nUnitWidth_ > 0)
		...
		nlbPixels = max (cbWidth, nlbPixels);
	}
	else if (nUnitWidth_ < 0){
		nlbPixels = UnitsToPixels (nUnitWidth_);
		nlbPixels = min (greatestExtent, nlbPixels);
		nlbPixels = max (cbWidth, nlbPixels);
	}
	...
	switch (fTCBAlign_){
	  ...
	  case eExpAlignLeft:
		  NewRectSize.right = __min(NewRectSize.right, ParentRect.right - nThumb);
		  break;
	  case eExpAlignRight:		 {
		  int nShift = NewRectSize.Width() - cbRect.Width();
		  OffsetRect(&NewRectSize, -nShift, 0);
		  NewRectSize.left = __max (NewRectSize.left, ParentRect.left + nThumb);
								 }
								 break;
	...
	if (NewRectSize.bottom > ParentRect.bottom - nThumb){
		wWord |= WS_VSCROLL;
		NewRectSize.bottom = __min(NewRectSize.bottom, ParentRect.bottom - nThumb);
	}
	else
		wWord &= ~WS_VSCROLL;
