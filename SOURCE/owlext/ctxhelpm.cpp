...
TCtxHelpFileManager::TCtxHelpFileManager(const owl_string& helpFileName)
:
...
void TCtxHelpFileManager::SetContextHelp(bool contextHelp)
{
	if (ContextHelp != contextHelp) {
		...
		if (ContextHelp) {
			...
			if (statusBar) {
				statusBar->UnBlock();
				statusBar->SetHintText(0); // Restore text of status bar
			}
		}
	}
}

void TCtxHelpFileManager::EvHelp(HELPINFO* hi)
...
void TCtxHelpStatusBar::SetHintText(LPCTSTR text)
...
void TCtxHelpStatusBar::EvLButtonDown(uint modKeys, TPoint& point)
{
