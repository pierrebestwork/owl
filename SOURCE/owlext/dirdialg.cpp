...
TDialog(parent, IDD_DIRDLG,
		...
		Data(data)
{
	// pointers for the TTreeWindow and the static control where
	// the current directory is displayed
	DirTree    = new TTreeWindow(this, IDC_DIRTREE);
...
{
	TDialog::SetupWindow();
	DirTree->SetImageList(TTreeWindow::Normal, *Images);
	LoadTree();
}
...
void
TDirDialog::TvnSelChanged(TTwNotify &)
{
...
// handler for the tree window.
void
TDirDialog::TvnItemExpanded(TTwNotify & twn)
{
	// get the node that was expanded or contracted
	TTreeNode node(*DirTree, twn.itemNew.hItem);

	// get the TTreeItem for the node
	TTreeItem item;
	// must do this so that TTreeItem knows we will be using the
	// item data
	item.SetItemData(0);
	node.GetItem(&item);
...
void
TDirDialog::BuildPath(owl_string& path, TTreeNode& node)
{
	...
	// create a TTreeItem with the buffer and get the item
	// from the node. 'buff' will now contain the directory name.
	buff[0] = _T('\0');
	TTreeItem item(buff, MAX_PATH);
	node.GetItem(&item);
...
TDirDialog::AddNode(LPCTSTR path, const TTreeNode& node, int mode)
{
	...
	if (path) {
		...
		// Add the node by creating a TTreeItem based on the string,
		// the image, and the select image. Use InsertChild so that
		// the list is sorted as we go.
		newNode = node.InsertChild( TTreeItem(buff, image1, image2),
			TTreeNode::Sort );
	}
