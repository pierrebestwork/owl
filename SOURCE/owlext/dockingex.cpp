												  ...
												  TPoint& p1, TPoint& p2, TPlacement& placement)
{
	...
	while(gadget) {
		...
		refbottom = max (refbottom, (int)gadbounds.bottom);
		maxright  = max (maxright, (int)gadbounds.right);
		if (pt.y <= refbottom) {
...
#if (OWLInternalVersion >= 0x06000000L)
void TDockableControlBarEx::EvOwlWindowDocked(uint /*pos*/, TDockingSlip* /*slip*/)
{
...
#endif
		if (GetAndSetCustomizeGadget(point)) {
...
void TDockableControlBarEx::EvRButtonDown(uint modKeys, TPoint& point)
{
	THarborEx* harbor = TYPESAFE_DOWNCAST(GetHarbor(), THarborEx);
	if (GetAndSetCustomizeGadget(point)) {
...
void TDockableControlBarEx::LoadSettings(THarborManagement& harborMan)
{
	...
	owl_string toolbarstr = harborMan.GetRegistryName() + owl_string(_T("\\")) +
		ToolbarsSection + owl_string(_T("\\")) + idstr;
	...
	for (int i=1; i<=iGadgetCount; i++) {
		...
		if (gadget) {
			...
			if (btg && THarborManagement::GetInternVersion() >= 2) {
				...
				owl_string tbBtnStr = toolbarstr+owl_string(_T("\\"))+ButtonSection+posstr;
				...
				if (tbBtnRegKey.QueryValue(
					TextSection, 0, (uint8*)(TCHAR*)text, &sizeText) == ERROR_SUCCESS) {
						btg->SetCommandText(owl_string(text));
						foundBtnStr = true;
				}
...
void TDockableControlBarEx::SaveSettings(THarborManagement& harborMan)
{
	...
	while (gadget) {
		...
		if (btg && btg->HasDefaultValues() == false) {
			...
			if (btgd && btgd->BmpResId != (uint32)(_TCHAR*)btg->GetResId()) {
				tbBtnRegKey.SetValue(BmpIdSection, (uint32)(_TCHAR*)btg->GetResId());
			}
			...
			if (btg->GetCommandText() != btgd->Text) {
				owl_string commandtext = btg->GetCommandText();
				tbBtnRegKey.SetValue(TextSection, REG_SZ,
					(const uint8*)commandtext.c_str(), _tcslen(commandtext.c_str()));
			}
		}
		i++;
		gadget = gadget->NextGadget();
...
void TFloatingSlipEx::EvNCRButtonDown(uint hitTest, TPoint& point)
{
...
void TEdgeSlipEx::EvWindowPosChanging(WINDOWPOS & windowPos)
// Copied more or less from TEdgeSlip implemenation
{
