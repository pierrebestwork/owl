...
TButtonGadgetEx::SetResId(TResId glyphResIdOrIndex)
{
	...
	if (SharingGlyph) {
		...
		ResId = glyphResIdOrIndex.IsString() ?
			strnewdup(glyphResIdOrIndex) : (_TCHAR *)glyphResIdOrIndex;
	}
	CelArray = 0;
...
bool TButtonGadgetEx::HasDefaultValues()
{
	TButtonGadgetDesc* bgd = TYPESAFE_DOWNCAST(
		GadgetDescriptors->Find(GetId()), TButtonGadgetDesc);
	if (bgd && ((_TCHAR *)bgd->BmpResId) == ResId)
		return true;
	return false;
}
...
TButtonGadgetEx::CommandEnable()
{
	...
	if (Window->GetHandle()) {
		Window->HandleMessage(
			...
			TParam2(&TButtonGadgetEnablerEx(*Window, this))
			);
	}
}
...
//
void
TButtonGadgetEx::MouseEnter(uint modKeys, TPoint& point)
{
	//
...
//
void
TButtonGadgetEx::Activate(TPoint& pt)
{
	switch (Type) {
case Exclusive:
	if (State != Down)
...
void TButtonTextGadgetEx::SetCommandText(const owl_string& commandText)
{
	CommandText = commandText;
	if (Window)
		Window->GadgetChangedSize(*this);
...
//
void TPopupButtonGadgetEx::BeginPressed(TPoint& p)
{
	PRECONDITION (Window);
...
//
void TPopupButtonGadgetEx::CancelPressed(TPoint& p, bool)
{
	if (Pressed)
...
//
void TPopupButtonGadgetEx::LButtonDown(uint modKeys, TPoint& point)
{
...
//
void TPopupButtonGadgetEx::LButtonUp(uint modKeys, TPoint& point)
{
...
void TColorButtonGadgetEx::Activate(TPoint& pt)
{
...
TControlGadgetEx::Created()
{
	...
	if (Control->GetHandle()) {
		owl_string text = GetHintText(Control, GetId());
		if (text.length()) {
...
TControlGadgetEx::Inserted()
{
	...
	//
	TGadgetDesc* desc = GadgetDescriptors->Find(Control->Attr.Id);
