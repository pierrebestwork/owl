		...
		if (PopupMenu) {
			...
			if (::GetMenuString(PopupMenu, 0, str, 255, MF_BYPOSITION) != 0) {
				::InsertMenu(me->GetMenu(), me->GetPosition()+offset,
					MF_BYPOSITION | MF_POPUP, (UINT)GetSubMenu(PopupMenu, 0), str);
			}
		}
	}
}
...
void THarborManagement::LoadSettings()
{
	...
	owl_string str = RegistryName + owl_string(_T("\\")) + MainWindowSection;
	...
	//
	if (InternVersion >= 5) {
		owl_string str2 = RegistryName + owl_string(_T("\\")) + ColorPickerSection;
...
//   is not always at bottom of the decorated frame
{
	...
	CutPasteCursor = gModule->LoadCursor(IDC_CUTPASTEGADGET);
	CopyCursor = gModule->LoadCursor(IDC_COPYGADGET);
	DeleteCursor = gModule->LoadCursor(IDC_DELETEGADGET);
...
//
void THarborEx::EvMouseMove(uint modKeys, TPoint& point)
{
	if (GadgetDragging && DragScreenDC) {
		TPoint originalPoint = point;
		...
		THarbor::EvMouseMove(modKeys, point);
}

void THarborEx::EvLButtonUp(uint modKeys, TPoint& point)
{
	if (GadgetDragging && DragScreenDC) {
		GadgetDragging = false;
...
void THarborEx::EvLButtonDblClk(uint modKeys, TPoint& point)
{
...
{
	if (idx >= 0 && idx < DCBData.GetItemsInContainer() &&
...
// Don't forget to destroy the menu by caller
{
	...
	if ((HMENU)PopupMenu && PopupMenu.GetSubMenu(0)) {
		uint num = __min(DCBData.GetItemsInContainer(), (uint)MaxDCBMenuItems);
		for (uint i=0; i<num; i++) {
			...
			//            -------------------
			//            &Customize...
			int len = _tcslen(DCBData[i]->Title);
...
void THarborEx::LoadSettings(THarborManagement& harborMan)
{
	...
	owl_string str = harborMan.GetRegistryName() + owl_string(_T("\\")) + ToolbarsSection;
	TRegKey regKey(TRegKey::GetCurrentUser(), str.c_str());
