...
#		if __DEBUG > 0
static LPCTSTR OwlExtName = _T("OWLX3DF.DLL");
#		else
static LPCTSTR OwlExtName = _T("OWLX3F.DLL");
#		endif
#	else
#		if __DEBUG > 0
static LPCTSTR OwlExtName = _T("OWLX3D.DLL");
#		else
static LPCTSTR OwlExtName = _T("OWLX3.DLL");
#		endif
#	endif
#endif
...
#	if defined(BI_PLAT_WIN32)
#		if __DEBUG > 0
static LPCTSTR OwlExtName = _T("OWLX3DV.DLL");
#		else
static LPCTSTR OwlExtName = _T("OWLX3V.DLL");
#		endif
#	else
#		error MS VC++ dosn't support 16 Bit
#	endif
#endif
...
TOWLEXTDll::Instance()
{
	...
	// performance increase to check the global gModule TModule pointer,
...
int OWLEXTEntryPoint(HINSTANCE hInstance, uint32 reason, LPVOID)
{
	...
	case DLL_PROCESS_DETACH:
		{
			// Assume that ::Module is a good value
			//delete gModule;  // bug
			return 1;
		}
	}
...
#endif
{
	return __OWLEXT OWLEXTEntryPoint(hInstance, reason, lpreserved);
}
...
GetOWLEXTModule()
{
	return &(__OWLEXT TOWLEXTDll::Instance());
}
...
owl_string GetHintText(TWindow* win, uint id, THintText hintType)
// Search the parent frame window of win and return the result of GetHintText
{
	...
	if (frame)
		frame->GetHintText(id, szBuf, COUNTOF(szBuf), hintType);
	return owl_string(szBuf);
}
//
