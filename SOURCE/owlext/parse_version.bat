...
: This script parses the OWLExt version number from the sources.
: It assumes the source file contains a single definition of the version number 
: formatted as follows:
: 
:   define _OWLEXT_VERSION(v) v(Major,Minor,Revision,Build)
: 
...
for /F "skip=2 tokens=5-8 delims=(,) " %%i in ('find "define _OWLEXT_VERSION(v)" ..\..\include\owlext\version.h') do set OWLVER=%%i%%j
