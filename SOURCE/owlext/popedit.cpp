...
TButton(parent, 0, _T(""), 0, 0, 0, 0, false, module)
{
	...
	if (TYPESAFE_DOWNCAST(Parent, TGadgetWindow))
		PopupEdit->Attr.H = BmpArray->CelSize().cy;
}
...
void TPopupButton::EvDrawItem(uint /*ctrlId*/, DRAWITEMSTRUCT & drawInfo)
{
	TRect bmpRect;
...
void TPopupButton::EvMouseMove(uint modKeys, TPoint& point)
{
...
void TPopupEdit::EvMouseMove(uint modKeys, TPoint& point)
{
	if (TYPESAFE_DOWNCAST(Parent, TGadgetWindow)){
		MapWindowPoints(*Parent, &point, 1);
		Parent->SendMessage(WM_MOUSEMOVE, (WPARAM)modKeys, MAKELPARAM(point.x, point.y));
	}
	else
...
void TPopupEdit::EvSize(uint /*sizeType*/, TSize& /*size*/)
{
...
void TPopupEdit::SetupWindow()
{
	...
	if (TYPESAFE_DOWNCAST(Parent, TGadgetWindow)){
		Font = new TFont(_T("MS Sans Serif"), -8);
...
void TPopupEditGadget::MouseEnter(uint modKeys, TPoint& pt)
{
...
void TPopupEditGadget::MouseLeave(uint modKeys, TPoint& pt)
{
