...
TProgressThread::TProgressThread(UINT id, HWND hTarget) :
TThread(),
_hTarget(hTarget)
, _id(id)
{ }
...
void TProgressThread::NotifyStart(uint32 len)
{
	if (_hTarget)
		::PostMessage(_hTarget, WM_GSC_PROGRESS_START, (WPARAM)_id, (LPARAM)len);
}

void TProgressThread::NotifyProgress(uint32 curr)
{
	if (_hTarget)
		::PostMessage(_hTarget, WM_GSC_PROGRESS, (WPARAM)_id, (LPARAM)curr);
}

void TProgressThread::NotifyEnd()
{
	if (_hTarget)
		::PostMessage(_hTarget, WM_GSC_PROGRESS_END, (WPARAM)_id, 0);
}
