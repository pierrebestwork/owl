...
//------------------------------------------------------------------------------
//@mfunc GetTipName()
owl_string TTipDialog::GetTipName()
{
	...
	*lp = 0;
	...
	return owl_string((_TCHAR*)buffer);
}
//------------------------------------------------------------------------------
//@mfunc CmNextTip()
...
void TTipDialog::OpenTipFile(long filePos)
{
	...
	if(!TipFile.rdbuf()->is_open()){
		...
		if(TipFile.fail()){
			TipError = true;
			MessageBox(GetModule()->LoadString(IDS_TIPDLG_ERRFILE_CORRUPT).c_str(), MB_OK);
		}
		else
			GetNextTipString();
	}
...
long TTipDialog::ReadSettings()
{
	RegKey += _T("\\"); 
	RegKey += szSection;
	TRegKey pTipReg((HKEY)TRegKey::CurrentUser, RegKey.c_str());
	long filePos;
...
{
#if !defined(BI_STD_STRING)
	CurrentTip.read_line(TipFile);
	while(!TipFile.eof() && *CurrentTip.c_str() == _T(';'))
		CurrentTip.read_line(TipFile);
#else
	...
	if(TipFile.eof()){
...
#if 0
		TipFile.seekg(0, ios::beg); //  why it isn't work ?????
#else
		TipFile.rdbuf()->close();                
		...
		TipError = true;
		MessageBox(GetModule()->LoadString(IDS_TIPDLG_ERRFILE_CORRUPT).c_str());
	}
}
//------------------------------------------------------------------------------
...
void TTipDialog::EvPaint ()
{
	...
	owl_string msg = GetModule()->LoadString(IDS_TIP_DIDYOUKNOW);
	rect.left += BulbBitmap->Width();
	TPoint pt1 = rect.TopLeft() + TPoint(0, BulbBitmap->Height() / 2);
	dc.ExtTextOut(pt1, ETO_OPAQUE, 0, msg.c_str(), -1);
