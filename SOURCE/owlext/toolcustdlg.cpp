...
void TToolbarInputDialog::CmOk()
{
	...
	if (Harbor->IsDCBTitleValid(Buffer) == false) {
		TCHAR message[255];
		GetModule()->LoadString(IDS_TOOLBAREXIST, message, 255);
		MessageBox(message, GetModule()->GetName(), MB_OK | MB_ICONEXCLAMATION);
	}
	else
		TInputDialog::CmOk();
}
	...
	if (TToolbarInputDialog(Harbor, this, title, prompt,
		buffer, MaxTitleLen).Execute() == IDOK) {
			...
			if (id != -1) {
				...
				for (uint i=0; i<Harbor->DCBData.GetItemsInContainer(); i++) {
					if (Harbor->DCBData[i]->DCB->Attr.Id == id) {
...
void TToolbarPage::CmDeleteToolbar()
{
	...
	//
	TCHAR message[128];
	GetModule()->LoadString(IDS_DELETETOOLBAR, message, sizeof(message));
	if (MessageBox(message, 0, MB_ICONEXCLAMATION | MB_OKCANCEL) == IDOK) {
...
//
void TCommandsListBox::EvLButtonDown(uint modKeys, TPoint& point)
{
...
void TCommandsPage::SetDescription(int id)
{
	if (id != DescId) {
		...
		owl_string str = GetHintText(this, id, htStatus);
		if (str.length() && id != 0)
	...
	if (num > 0) {
		...
		for (i=0, j=0; i<GadgetDescriptors->Array.GetItemsInContainer(); i++) {
			...
			if (desc && desc->Attr & GADG_ISCOMMAND) {
				...
				CommandsListBox->SetItemData(j, (uint32)desc);
				j++;
			}
		}
	}
...
void TCommandsPage::ShowCommands(owl_string name)
{
	uint i,j;
	for (i=0, j=0; i<GadgetDescriptors->Array.GetItemsInContainer(); i++) {
		TGadgetDesc* desc = GadgetDescriptors->Array[i];
