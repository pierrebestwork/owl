...
CDEFINES       = -D_OWLFXPCH -D_OWLPCH -DBI_NOINCLUDE_NS_OWL
DLLCDEFS     = -D_BUILD_OWLFXDLL -D_OWLDLL -D_RTLDLL 
LIBCDEFS     = -D_BUILD_OWLFXLIB
...
!  else
!    if $(BCCVER) == 8
       CFLAGS = $(CFLAGS) -3 -v -k -Ogmpv     # Optimizations -O1gmpv , stdcall, debug info on - to fix a problem with VIRDEF conflicts
!    else
...
# VH, May 2006:
# The resource file (owlextall.res) needs preprocessing of "version.rc".
...
# Then "owlextall.rc" will include "version.i" instead (see "owlextall.rc").

version.i: version.rc $(OWLROOT)\include\owlext\version.h
...
DESCRIPTION 'OWL Extensions Class Library'
EXETYPE     WINDOWS
DATA        MULTIPLE
EXPORTS
!if $d(USE_ORD32)
| + $(.PATH.cpp)\owlfx32.ord $(DEFFILE) > NUL
!else
| $@ > NUL
!endif
