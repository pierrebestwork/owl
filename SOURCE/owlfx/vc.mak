#
# vc.mak - Makefile for building owlfx with Microsoft Visual C++
#
...
!if "$(DLL)" == "1"
DLL_OPT=/D_DLL /D_BUILD_OWLFXDLL /D_OWLDLL
#!else
#LIB_OPT=/DBUILD_OWLFXLIB
!endif

CL_OPT=/D_OWLFXPCH /D_OWLPCH
CPPFLAGS=/D_OWLFXPCH /D_OWLPCH
PCH_HEADER="owlfx/pch.h"
...
$(PCH_TARGETS):: msc\$(PCH_CPP).cpp $(HDRS)
	$(MSCL) @<<
/Yc /Fp$(PCH_FILE) $(CL_MODEL) $(CL_OPT) $(DLL_OPT) $(LIB_OPT) $(PDBOPTS) $(DEFS) $(OPT) /D_OWLPCH /D_OWLFXPCH /c msc\$(PCH_CPP).cpp
<<
!if "$(BROWSE)" != "0"
...
CPPFLAGS_STATIC=\
	$(CL_MODEL) $(CL_OPT) $(DEFS) $(DEBDEF) /I$(OWLROOT)\include /D_BUILD_OWLFXDLL /D_OWLFXDLL /D_OWLFXPCH /D_OWLDLL /D_OWLPCH $(OPT) $(EH) /Gy /Fd$(LIBDIR)\$(GOAL)i.pdb
...
PCH_TARGET=$D\owlfxpchs.obj

$D\owlfxpchs.obj $D\owlfxpchs.pch: msc\owlfxpchs.cpp 
	$(MSCL) @<<
/c $(CPPFLAGS_STATIC) /Yc /Fp$D\owlfxpchs.pch msc\owlfxpchs.cpp
<<

#-------------------------------------------------
# Build target

$D\$(LIBNAME).res: owlfx.rc
	$(MSRC) $(RCFLAGS) $(RCDEFINES) /i$(OWLROOT)\include;$(OWLROOT)\include\owl;$(OWLROOT)\include\owlfx /fo $D\$(LIBNAME).res owlfx.rc
